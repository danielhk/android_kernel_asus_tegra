 ...
 */
int sd_get_rx_unit(bt_private * priv)
{
 ...
 *  @return	   BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int sd_read_firmware_status(bt_private * priv, u16 * dat)
{
	int ret = BT_STATUS_SUCCESS;
	u8 fws0;
	u8 fws1;
 ...
 */
static int sd_read_rx_len(bt_private * priv, u16 * dat)
{
	int ret = BT_STATUS_SUCCESS;
	u8 reg;
	struct sdio_mmc_card *card = (struct sdio_mmc_card *) priv->bt_dev.card;
 ...
 *  @return	   BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int sd_enable_host_int_mask(bt_private * priv, u8 mask)
{
	int ret = BT_STATUS_SUCCESS;
	struct sdio_mmc_card *card = (struct sdio_mmc_card *) priv->bt_dev.card;
 ...
 *  @return	   BT_STATUS_SUCCESS or other error no.
 */
static int sd_disable_host_int_mask(bt_private * priv, u8 mask)
{
 ...
 *  @return		BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int sd_poll_card_status(bt_private * priv, u8 bits)
{
	int tries;
	int rval;
	struct sdio_mmc_card *card = (struct sdio_mmc_card *) priv->bt_dev.card;
 ...
 */
static int sd_probe_card(struct sdio_func *func,
 ...
 *  @return         BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
int sd_verify_fw_download(bt_private * priv, int pollnum)
{
	int ret = BT_STATUS_SUCCESS;
	u16 firmwarestat;
	int tries;
 ...
 *  @return	     BT_STATUS_SUCCESS/BT_STATUS_FAILURE or other error no.
 */
static int sd_init_fw_dpc(bt_private * priv)
{
	struct sdio_mmc_card *card = (struct sdio_mmc_card *) priv->bt_dev.card;
	u8 *firmware = NULL;
	int firmwarelen;
	...
	do {
		...
		if (len & BIT(0)) {
			...
			tx_blocks =
				(txlen + SD_BLOCK_SIZE_FW_DL - 1) / SD_BLOCK_SIZE_FW_DL;
 ...
 * @return             BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int sd_request_fw_dpc(const struct firmware *fw_firmware,
							 void *context)
{
	int ret = BT_STATUS_SUCCESS;
 ...
 * @return             None
 **/
static void sd_request_fw_callback(const struct firmware *firmware,
								   void *context)
{
	ENTER();
 ...
 *  @return		BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int sd_card_to_host(bt_private * priv)
{
	int ret = BT_STATUS_SUCCESS;
	u16 buf_len = 0;
	int buf_block_len;
	...
	if ((t_ptr) skb->data & (DMA_ALIGNMENT - 1)) {
		skb_put(skb, DMA_ALIGNMENT - ((t_ptr) skb->data & (DMA_ALIGNMENT - 1)));
		skb_pull(skb, DMA_ALIGNMENT - ((t_ptr) skb->data & (DMA_ALIGNMENT - 1)));
	}
			...
			if (priv->debug_device_pending) {
				if (priv->debug_ocf_ogf[0] == skb->data[3] &&
					...
					if (debug_dev) {
						...
						mdev_recv_frame(skb);
						mdev_debug->stat.byte_rx += buf_len;
					}
					break;
				}
			}
 ...
 *  @return        N/A
 */
static void sd_remove_card(struct sdio_func *func)
{
 ...
 *  @return      N/A
 */
static void sd_interrupt(struct sdio_func *func)
{
	bt_private *priv;
 ...
 *  @return       BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
int sd_check_winner_status(bt_private * priv, u8 * val)
{
 ...
 *  @return	   BT_STATUS_SUCCESS or other error no.
 */
int bt_sdio_suspend(struct device *dev)
{
 ...
 *  @return	   BT_STATUS_SUCCESS
 */
int bt_sdio_resume(struct device *dev)
{
	struct sdio_func *func = dev_to_sdio_func(dev);
	mmc_pm_flag_t pm_flags = 0;
	bt_private *priv = NULL;
