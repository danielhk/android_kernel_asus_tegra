 ...
 */
ssize_t chardev_write(struct file *filp, const char *buf,
 ...
 *	@return			0--success otherwise failure
 */
int chardev_ioctl(struct inode *inode, struct file *filp,
					  ...
					  char *mod_name, char *dev_name)
{
	...
	if ((dev->dev_type == BT_TYPE) || (dev->dev_type == BT_AMP_TYPE)) {
		device_create(char_class, NULL, MKDEV(mbtchar_major, dev->minor),
					  NULL, dev_name);
	}
