        ...
        if ((priv->bt_dev.sendcmdflag == TRUE) &&
            ...
            if (pEvent->data[1] == MODULE_BRINGUP_REQ) {
                ...
                PRINTM(CMD, "devFeature:  %s,    %s,    %s,    %s,    %s\n",
                       ((pEvent->
                         data[4] & DEV_FEATURE_BT) ? "BT Feature" :
                        "No BT Feature"),
                       ((pEvent->
                         data[4] & DEV_FEATURE_BTAMP) ? "BTAMP Feature" :
                        "No BTAMP Feature"),
                       ((pEvent->
                         data[4] & DEV_FEATURE_BLE) ? "BLE Feature" :
                        "No BLE Feature"),
                       ((pEvent->
                         data[4] & DEV_FEATURE_FM) ? "FM Feature" :
                        "No FM Feature"),
                       ((pEvent->
                         data[4] & DEV_FEATURE_NFC) ? "NFC Feature" :
                        "No NFC Feature"));
            }
...
sbi_register_conf_dpc(bt_private * priv)
{
    ...
    if (mbt_dev) {
        ...
        register_char_dev(char_dev, chardev_class, MODULE_NAME, mbt_dev->name);
    ...
    if ((drv_mode & DRV_MODE_FM) &&
        ...
        fmchar_minor++;
                ...
                /** register char dev */
        register_char_dev(char_dev, chardev_class, MODULE_NAME, fm_dev->name);
    ...
    if ((drv_mode & DRV_MODE_NFC) &&
        ...
        nfcchar_minor++;
                ...
                /** register char dev */
        register_char_dev(char_dev, chardev_class, MODULE_NAME, nfc_dev->name);
