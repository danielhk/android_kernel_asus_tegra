...
/** Version */
#define VERSION "M2614108"

/** Driver version */
char mbt_driver_version[] =
	"SD8797-%s-" VERSION
	"-(" "FP" FPNUM ")"
#ifdef DEBUG_LEVEL2
	"-dbg"
 ...
 */
struct debug_dev *alloc_debug_dev(void)
{
...
int check_evtpkt(bt_private *priv, struct sk_buff *skb)
{
	...
	if (hdr->evt == HCI_EV_CMD_COMPLETE) {
        ...
        case FM_CMD:
            {
                u8 *pos = (skb->data + HCI_EVENT_HDR_SIZE + sizeof(struct hci_ev_cmd_complete) + 1);
		...
		if ((priv->bt_dev.sendcmdflag == TRUE) &&
			...
			if (pEvent->data[1] == MODULE_BRINGUP_REQ) {
				PRINTM(CMD, "BT: EVENT %s:%s\n", m_dev->name,
					...
					MODULE_CFG_RESP_ALREADY_UP)) ? "Bring up Fail" : "Bring up success");
				priv->bt_dev.devType = pEvent->data[3];
				PRINTM(CMD, "devType:%s\n",
					(pEvent->data[3] == DEV_TYPE_AMP) ? "AMP controller" : "BR/EDR controller");
				priv->bt_dev.devFeature = pEvent->data[4];
				PRINTM(CMD, "devFeature:  %s,    %s,    %s,    %s,    %s\n",
					((pEvent->data[4] & DEV_FEATURE_BT) ? "BT Feature" : "No BT Feature"),
					((pEvent->data[4] & DEV_FEATURE_BTAMP) ? "BTAMP Feature" : "No BTAMP Feature"),
					((pEvent->data[4] & DEV_FEATURE_BLE) ? "BLE Feature" : "No BLE Feature"),
					((pEvent->data[4] & DEV_FEATURE_FM) ? "FM Feature" : "No FM Feature"),
					((pEvent->data[4] & DEV_FEATURE_NFC) ? "NFC Feature" : "No NFC Feature"));
			}
			if (pEvent->data[1] == MODULE_SHUTDOWN_REQ) {
				PRINTM(CMD, "BT: EVENT %s:%s\n", m_dev->name,
					(pEvent->data[2]) ? "Shut down Fail" : "Shut down success");
 ...
 *  @return         N/A
 */
static void bt_cmd_timeout_func(bt_adapter *adapter, u16 cmd)
{
	ENTER();
 ...
 *  @return    BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
int bt_send_sdio_pull_ctrl_cmd(bt_private *priv)
{
	struct sk_buff *skb = NULL;
	u8 ret = BT_STATUS_SUCCESS;
	BT_CMD *pCmd;
 ...
 *  @return    BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int SendSinglePacket(bt_private *priv, struct sk_buff *skb)
{
	int ret;
	ENTER();
	if (!skb || !skb->data) {
 ...
 *  @return    N/A
 */
static void bt_init_adapter(bt_private *priv)
{
	ENTER();
 ...
 *  @return    BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int bt_init_fw(bt_private *priv)
{
 ...
 *  @return    -ENOIOCTLCMD
 */
static int mdev_ioctl(struct m_dev *m_dev, unsigned int cmd,
					  unsigned long arg)
{
	ENTER();
	LEAVE();
 ...
 *  @return    N/A
 */
static void mdev_destruct(struct m_dev *m_dev)
{
	ENTER();
	LEAVE();
	return;
 ...
 */
static int mdev_send_frame(struct m_dev *m_dev, struct sk_buff *skb)
{
 ...
 *  @return    BT_STATUS_SUCCESS
 */
static int mdev_flush(struct m_dev *m_dev)
{
 ...
 *  @return    BT_STATUS_SUCCESS
 */
static int mdev_close(struct m_dev *m_dev)
{
	ENTER();
	mdev_req_lock(m_dev);
 ...
 */
static int mdev_open(struct m_dev *m_dev)
{
	ENTER();
 ...
 *  @return        BT_STATUS_SUCCESS
 */
static int bt_service_main_thread(void *data)
{
...
int sbi_register_conf_dpc(bt_private *priv)
{
	...
	if (mbt_dev) {
		...
		if (bt_name)
			...
			snprintf(mbt_dev->name, sizeof(mbt_dev->name), "mbtchar%d",
				 mbtchar_minor);
		...
		snprintf(priv->bt_dev.m_dev[BT_SEQ].name,
		    sizeof(priv->bt_dev.m_dev[BT_SEQ].name), mbt_dev->name);
	...
	if ((drv_mode & DRV_MODE_NFC) &&
		...
		snprintf(priv->bt_dev.m_dev[NFC_SEQ].name,
		    sizeof(priv->bt_dev.m_dev[NFC_SEQ].name), nfc_dev->name);
	...
	if ((debug_intf) &&
		...
		if (debug_name)
			snprintf(debug_dev->name, sizeof(debug_dev->name), "%s%d",
				 debug_name, debugchar_minor);
		else
			snprintf(debug_dev->name, sizeof(debug_dev->name), "mdebugchar%d",
				 debugchar_minor);
		snprintf(dev_file, sizeof(dev_file), "/dev/%s", debug_dev->name);
		debugchar_minor++;
		...
		snprintf(priv->bt_dev.m_dev[DEBUG_SEQ].name,
		    sizeof(priv->bt_dev.m_dev[DEBUG_SEQ].name), debug_dev->name);
		...
		if (m_dev->spec_type == IANYWHERE_SPEC) {
			...
			for (i = 0; i < 3; i++)
				kfree_skb(((struct mbt_dev *)
					(m_dev->dev_pointer))->reassembly[i]);
			/**  unregister m_dev to char_dev */
			if (chardev_class)
				chardev_cleanup_one(m_dev, chardev_class);
			free_m_dev(m_dev);
 ...
 *  @return    BT_STATUS_SUCCESS or BT_STATUS_FAILURE
 */
static int bt_init_module(void)
{
	int ret = BT_STATUS_SUCCESS;
	ENTER();
	bt_root_proc_init();
 ...
 *  @return        N/A
 */
static void bt_exit_module(void)
{
	ENTER();
...
MODULE_PARM_DESC(fw,
		"0: Skip firmware download; otherwise: Download firmware");
...
MODULE_PARM_DESC(debug_intf, "1: Enable debug interface; 0: Disable debug interface ");
