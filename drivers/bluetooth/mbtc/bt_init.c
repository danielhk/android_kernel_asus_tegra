 ...
 */
static char *bt_strsep(char **s, char delim, char esc)
{
	char *se = *s, *sb;
 ...
 *  @return	N/A
 */
static void bt_mac2u8(u8 *mac_addr, char *buf)
{
	char *begin = buf, *end;
	int i;
 ...
 */
static int bt_atoi(int *data, char *a)
{
	int i, val = 0, len;
...
int bt_process_init_cfg(bt_private *priv, u8 *data, u32 size)
{
	...
	printk(KERN_ERR "++++++ enter  bt_process_init_cfg()++++++\n");
	...
	while ((line_len = parse_cfg_get_line(data, size, s)) != -1) {
		...
		if (strncmp(pos, "mac_addr", 8) == 0) {
			...
			if (intf_s != NULL && intf_e != NULL) {
				...
				strncpy(dev_name, intf_s + 1, intf_e - intf_s - 1);
				dev_name[intf_e - intf_s - 1] = '\0';
				if (strcmp(dev_name, priv->bt_dev.m_dev[BT_SEQ].name) == 0) {
					/* found hci device */
					...
					PRINTM(CMD,
						   ...
						   dev_name, bt_mac[0], bt_mac[1], bt_mac[2],
	...
	LEAVE();
	printk(KERN_ERR "++++++ leave  bt_process_init_cfg()++++++\n");
	return ret;
}
...
int bt_init_config(bt_private *priv, char *cfg_file)
{
	...
	ENTER();
	printk(KERN_ERR "++++++  enter   bt_init_config()++++++\n");
	...
	LEAVE();
	printk(KERN_ERR "+++++ return val bt_init_config is %d ++++\n", ret);
	printk(KERN_ERR "++++++  leave   bt_init_config()++++++\n");
	return ret;
}
