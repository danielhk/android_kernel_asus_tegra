...
static struct item_data status_items[] = {
    {"version", item_adapter_size(drv_ver), 0, item_adapter_addr(drv_ver), OFFSET_BT_ADAPTER|SHOW_STRING},
 ...
 */
static int form_cmd52_string(bt_private *priv)
{
	ENTER();
 ...
 *  @return	BT_STATUS_SUCCESS
 */
static int parse_cmd52_string(const char __user *buffer, size_t len,
 ...
 *  @return		N/A
 */
static void proc_on_close(struct inode *inode, struct file *file)
{
...
static int proc_open(struct inode *inode, struct file *file)
{
	...
	for (i = 0; i < priv->num_items; i++) {
		...
		if (priv->pdata[i].flag & SHOW_INT)
			...
			if (!strncmp
				(priv->pdata[i].name, "sdcmd52rw", strlen("sdcmd52rw"))) {
...
int bt_proc_init(bt_private *priv, struct m_dev *m_dev, int seq)
{
	...
	memset(cmd52_string, 0, CMD52_STR_LEN);
	if (proc_mbt) {
		priv->dev_proc[seq].proc_entry = proc_mkdir(m_dev->name, proc_mbt);
		...
		priv->dev_proc[seq].pfiles =
			(struct proc_private_data *) kmalloc(sizeof(proc_files), GFP_ATOMIC);
		...
		for (j = 0; j < priv->dev_proc[seq].num_proc_files; j++) {
			priv->dev_proc[seq].pfiles[j].pdata =
				(struct item_data *) kmalloc(priv->dev_proc[seq].pfiles[j].
							num_items *
			...
			memcpy((u8 *) priv->dev_proc[seq].pfiles[j].pdata,
				   ...
				   sizeof(struct item_data));
			for (i = 0; i < priv->dev_proc[seq].pfiles[j].num_items; i++) {
				if (priv->dev_proc[seq].pfiles[j].pdata[i].
					flag & OFFSET_BT_DEV)
					priv->dev_proc[seq].pfiles[j].pdata[i].addr =
						priv->dev_proc[seq].pfiles[j].pdata[i].offset +
						(t_ptr) &priv->bt_dev;
				if (priv->dev_proc[seq].pfiles[j].pdata[i].
					flag & OFFSET_BT_ADAPTER)
					priv->dev_proc[seq].pfiles[j].pdata[i].addr =
						priv->dev_proc[seq].pfiles[j].pdata[i].offset +
						(t_ptr) priv->adapter;
			}
	...
	if (ret == BT_STATUS_FAILURE) {
		...
		if (priv->dev_proc[seq].pfiles) {
			for (j = 0; j < priv->dev_proc[seq].num_proc_files; j++) {
				if (priv->dev_proc[seq].pfiles[j].pdata) {
					kfree(priv->dev_proc[seq].pfiles[j].pdata);
					priv->dev_proc[seq].pfiles[j].pdata = NULL;
				}
			}
	...
	if (proc_mbt) {
		for (i = 0; i < MAX_RADIO_FUNC; i++) {
			if (priv->dev_proc[i].proc_entry) {
				for (j = 0; j < sizeof(proc_files) / sizeof(proc_files[0]);
					 j++)
			...
			if (priv->dev_proc[i].pfiles) {
				for (j = 0; j < priv->dev_proc[i].num_proc_files; j++) {
					if (priv->dev_proc[i].pfiles[j].pdata) {
						kfree(priv->dev_proc[i].pfiles[j].pdata);
						priv->dev_proc[i].pfiles[j].pdata = NULL;
					}
				}
