 ...
 */
static int woal_info_proc_read(char *page, char **start, off_t offset,
	       int count, int *eof, void *data)
{
    ...
    if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP) {
        ...
        if(MLAN_STATUS_SUCCESS != woal_uap_get_stats(priv, MOAL_PROC_WAIT,&ustats)) {
            *eof = 1;
            goto exit;
        }
    }
    ...
    for (i = 0; i < netdev->num_tx_queues; i++) {
        p += sprintf(p, "tx queue %d:  %s\n", i,
                ((netif_tx_queue_stopped(netdev_get_tx_queue(netdev, 0))) ? "stopped" : "started"));
    }
#else
 ...
 */
static int parse_cmd52_string(const char __user *buffer, size_t len, int *func, int *reg, int *val)
{
 ...
 */
static int woal_config_write(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    char databuf[101];
    char *line;
    t_u32  config_data = 0;
    ...
    if(!strncmp(databuf,"drv_mode",strlen("drv_mode"))){
        ...
        if(config_data != (t_u32) drv_mode)
            if (woal_switch_drv_mode(handle, config_data) != MLAN_STATUS_SUCCESS) {
 ...
 *  @return	   number of output data
 */
static int woal_config_read(char *page, char **s, off_t off, int cnt, int *eof, void *data)
{
    char *p = page;
    moal_handle *handle = (moal_handle *)data;
    ...
    p += sprintf(p, "sdcmd52rw=%d 0x%0x 0x%02X\n", handle->cmd52_func, handle->cmd52_reg, handle->cmd52_val);
    MODULE_PUT;
    LEAVE();
    return p-page;
}
...
void woal_create_proc_entry(moal_private *priv)
{
    ...
    if (priv->phandle->proc_mwlan && !priv->proc_entry) {
        ...
        if (priv->proc_entry) {
            create_proc_read_entry("info", 0, priv->proc_entry, woal_info_proc_read, dev);
        }
    }
