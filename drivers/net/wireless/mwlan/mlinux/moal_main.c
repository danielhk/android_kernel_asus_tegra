		...
		void *ptr)
{
	...
	if (priv->bss_type != MLAN_BSS_TYPE_STA) {
		PRINTM(MIOCTL, "Bss type [%d] is not STA, ignore\n",
				(int)priv->bss_type);
		ret = NOTIFY_DONE;
		goto done;
	}
	...
	memset(&misc->param.ipaddr_cfg, 0, sizeof(misc->param.ipaddr_cfg));

	switch (event) {
	case NETDEV_UP:
		misc->param.ipaddr_cfg.op_code = 3;
		...
		memcpy(misc->param.ipaddr_cfg.ip_addr[0],
				&ifa->ifa_address,
	...
	case NETDEV_DOWN:
		/* Remove ipaddr */
		misc->param.ipaddr_cfg.op_code = 0;
         ...
         if(memcmp(ssid_bssid->bssid, zero_mac, sizeof(zero_mac))) {
            if(ssid_bssid->ssid.ssid_len) {  //compare ssid and bssid
                if((ssid_bssid->ssid.ssid_len == bss_info.ssid.ssid_len) &&
                    !memcmp(ssid_bssid->ssid.ssid, bss_info.ssid.ssid,bss_info.ssid.ssid_len) &&
                    !memcmp(ssid_bssid->bssid, bss_info.bssid, MLAN_MAC_ADDR_LENGTH))
                    ret = MTRUE;
            }
            else {  //compare bssid
                if(!memcmp(ssid_bssid->bssid, bss_info.bssid, MLAN_MAC_ADDR_LENGTH)){
                    ret = MTRUE;
                }
            }
        }
        else {  //compare ssid
            if(ssid_bssid->ssid.ssid_len && (ssid_bssid->ssid.ssid_len == bss_info.ssid.ssid_len) &&
                !memcmp(ssid_bssid->ssid.ssid, bss_info.ssid.ssid,bss_info.ssid.ssid_len)){
                ret = MTRUE;
            }
       }
 ...
 */
static mlan_status woal_update_drv_tbl(moal_handle *handle, int drv_mode_local)
{
    ...
    if ((intf_num == 0) || (intf_num > MLAN_MAX_BSS_NUM)) {
        ...
        /* Create new table */
        if (!(bss_tbl = (mlan_bss_attr *) kmalloc(sizeof(mlan_bss_attr) * intf_num, GFP_KERNEL))) {
 ...
 *  @return	   MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status woal_init_sw(moal_handle *handle)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    unsigned int  i;
    mlan_device device;
    ...
    woal_initialize_timer(&handle->go_timer,
                          woal_go_timer_func,
                          handle);
    ...
    for (i = 0 ; i < handle->drv_mode.intf_num; i++) {
        ...
        device.bss_attr[i].bss_priority =  handle->drv_mode.bss_attr[i].bss_priority;
 ...
 *  @return	    N/A
 */
static void woal_free_moal_handle(moal_handle *handle)
{
	ENTER();
	if (!handle) {
    ...
    if(atomic_read(&handle->lock_count) || atomic_read(&handle->malloc_count) || atomic_read(&handle->mbufalloc_count)) {
	PRINTM(MERROR,"mlan has memory leak: lock_count=%d, malloc_count=%d, mbufalloc_count=%d\n",
               atomic_read(&handle->lock_count), atomic_read(&handle->malloc_count), atomic_read(&handle->mbufalloc_count));
    }
 ...
 */
static t_u32 woal_process_regrdwr(moal_handle *handle, t_u8 *type_string, t_u8 *offset_string, t_u8 *value_string)
{
	...
	if (MLAN_STATUS_SUCCESS != woal_request_ioctl(handle->priv[0],ioctl_req, MOAL_IOCTL_WAIT)) {
		goto done;
	}
...
static t_u32 woal_process_init_cfg(moal_handle *handle, t_u8 *data, t_size size)
{
	...
	while ((line_len = parse_cfg_get_line(data, size, s)) != -1) {
		...
		if (strncmp(pos, "mac_addr", 8) == 0) {
			...
			if (intf_s != NULL && intf_e != NULL) {
				...
				for (i = 0; i < handle->priv_num; i++) {
					if (strcmp(bss_mac_name, handle->priv[i]->netdev->name) == 0) {
						memset(handle->priv[i]->current_addr, 0, ETH_ALEN);
						PRINTM(MINFO, "Interface name: %s mac: %s\n", bss_mac_name, bss_mac_addr);
						woal_mac2u8(handle->priv[i]->current_addr, bss_mac_addr);
#if defined(WIFI_DIRECT_SUPPORT)
#if defined(STA_CFG80211) && defined(UAP_CFG80211)
#if LINUX_VERSION_CODE >= WIFI_DIRECT_KERNEL_VERSION
						if(handle->priv[i]->bss_type == MLAN_BSS_TYPE_WIFIDIRECT){
						...
						if (MLAN_STATUS_SUCCESS != woal_request_set_mac_address(handle->priv[i])) {
							PRINTM(MERROR, "Set MAC address failed\n");
							goto done;
						}
						memcpy(handle->priv[i]->netdev->dev_addr, handle->priv[i]->current_addr, ETH_ALEN);
		...
		/* Process REG value */
			...
			if (MLAN_STATUS_SUCCESS != woal_process_regrdwr(handle, type, offset, value)){
 ...
 *    @return             MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static t_u32 woal_set_user_init_data(moal_handle *handle)
{
    mlan_status ret = MLAN_STATUS_FAILURE;
	t_u8	*cfg_data = NULL;
	t_size	len;
	...
	if ((request_firmware(&handle->user_data, init_cfg, handle->hotplug_device)) < 0) {
...
static mlan_status woal_add_card_dpc(moal_handle *handle)
{
    ...
    for (i = 0; i < handle->drv_mode.intf_num; i++) {
        if(handle->drv_mode.bss_attr[i].bss_virtual)
            continue;
        if (!woal_add_interface(handle, handle->priv_num,  handle->drv_mode.bss_attr[i].bss_type)) {
            ret = MLAN_STATUS_FAILURE;
            goto err;
        }
    }
...
static mlan_status woal_init_fw_dpc(moal_handle *handle)
{
    ...
    if (cal_data_cfg) {
        if ((request_firmware(&handle->user_data, cal_data_cfg, handle->hotplug_device)) < 0 ) {
    ...
    if(handle->hardware_status != HardwareStatusReady) {
        woal_moal_debug_info(woal_get_priv(handle, MLAN_BSS_ROLE_ANY), handle, MTRUE);
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    ret = MLAN_STATUS_SUCCESS;
 ...
 * @return        MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status woal_request_fw_dpc(moal_handle *handle, const struct firmware *firmware)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 * @return        N/A
 */
static void woal_request_fw_callback(const struct firmware *firmware, void *context)
{
    ENTER();
...
mlan_status woal_request_fw(moal_handle *handle)
{
    ...
    if (req_fw_nowait) {
        ...
        if ((err = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                            handle->drv_mode.fw_name, handle->hotplug_device,
                            GFP_KERNEL, handle, woal_request_fw_callback)) < 0) {
#else
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,13)
        if ((err = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                            handle->drv_mode.fw_name, handle->hotplug_device,
                            handle, woal_request_fw_callback)) < 0) {
#else
...
#endif
#endif
            PRINTM(MFATAL, "WLAN: request_firmware_nowait() failed, error code = %d\n", err);
            ret = MLAN_STATUS_FAILURE;
        }
    } else {
        if ((err=request_firmware(&handle->firmware, handle->drv_mode.fw_name, handle->hotplug_device)) < 0) {
 ...
 *  @return	   MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status woal_init_fw(moal_handle *handle)
{
    mlan_status  ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return        N/A
 */
static void woal_fill_mlan_buffer(moal_private *priv,
                                  mlan_buffer *pmbuf,
                                  struct sk_buff *skb)
{
    struct timeval tstamp;
 ...
 */
moal_private *woal_add_interface(moal_handle *handle, t_u8 bss_index, t_u8 bss_type)
{
...
#define MAX_WMM_QUEUE   4
	...
	if ((bss_type == MLAN_BSS_TYPE_WIFIDIRECT) && (dev_alloc_name(dev, name) < 0)) {
...
#if defined(STA_WEXT) || defined(UAP_WEXT)
        if(IS_STA_OR_UAP_WEXT(cfg80211_wext) && GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_STA) {
 ...
 */
static mlan_status woal_shutdown_fw(moal_private * priv, t_u8 wait_option)
{
    mlan_ioctl_req      *req = NULL;
    mlan_ds_misc_cfg    *misc = NULL;
    mlan_status         status;
    ...
    /* Allocate an IOCTL request buffer */
    req = (mlan_ioctl_req *)woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_misc_cfg));
    if (req == NULL) {
 ...
 */
static int woal_hexval(char chr)
{
    if (chr >= '0' && chr <= '9')
        return chr - '0';
    if (chr >= 'A' && chr <= 'F')
 ...
 *  @return        N/A
 */
static void woal_terminate_workqueue(moal_handle *handle)
{
    ENTER();
 ...
 */
u16 woal_select_queue(struct net_device *dev,
					 struct sk_buff *skb)
{
	moal_private    *priv = (moal_private *) netdev_priv(dev);
	...
	index = mlan_select_wmm_queue(priv->phandle->pmlan_adapter,priv->bss_index,tid);
    ...
    if (ntohs(ethh->h_proto) == ETH_P_IP) {
        ...
        if (iph->protocol == IPPROTO_TCP) {
            ...
            if (tcph->fin) {
                ...
                if (tcp_sess != NULL) {
                    PRINTM(MINFO, "TX: release a tcp session in dl. (src: ipaddr 0x%08x, port: %d. dst: ipaddr 0x%08x, port: %d\n", iph->daddr, tcph->dest, iph->saddr, tcph->source);
...
#define TCP_ACK_RECV_WARMING_UP  1000 /* recovery */
    ...
    if (tcph->syn & tcph->ack) {
        ...
        PRINTM(MINFO, "create a tcp session. (src: ipaddr 0x%08x, port: %d. dst: ipaddr 0x%08x, port: %d\n", iph->saddr, tcph->source, iph->daddr, tcph->dest);
        ...
        if (tcp_sess != NULL) {
            /* remove the tcp session from the queue */
            PRINTM(MINFO, "Rx: release a tcp session in ul. (src: ipaddr 0x%08x, port: %d. dst: ipaddr 0x%08x, port: %d\n", iph->saddr, tcph->source, iph->daddr, tcph->dest);
            list_del(&tcp_sess->link);
            kfree(tcp_sess);
        }
        else {
...
void woal_init_priv(moal_private *priv, t_u8 wait_option)
{
    ...
    while (!list_empty(&priv->tcp_sess_queue))
    {
        ...
        PRINTM(MINFO, "warm reset: release a tcp session in dl. (src: ipaddr 0x%08x, port: %d. dst: ipaddr 0x%08x, port: %d\n", tcp_sess->src_ip_addr, tcp_sess->src_tcp_port, tcp_sess->dst_ip_addr, tcp_sess->dst_tcp_port);
        list_del(link);
        kfree(tcp_sess);
    }
...
pmlan_ioctl_req woal_alloc_mlan_ioctl_req(int size)
{
	...
	if(!(req = (mlan_ioctl_req *)kmalloc((sizeof(mlan_ioctl_req) + size + sizeof(int) +
    ...
    for (;;) {
        ...
        for (i = 0; i < handle->priv_num && (priv = handle->priv[i]); i++) {
            ...
            */
            ...
            if (MLAN_STATUS_SUCCESS != woal_request_scan(priv, MOAL_CMD_WAIT, &req_ssid)) {
		...
		mlan_ds_rate    *rate = NULL;
                ...
                if (priv->set_asynced_essid_flag == MTRUE) {
                    ...
                    if (MLAN_STATUS_SUCCESS != woal_get_bss_info(priv,
                        ...
                        PRINTM(MINFO, "Set asynced essid: Fail to get bss info after assoc\n");
                    } else {
...
#define SDIO_SCRATCH_REG 0x60
        ...
        for (reg=reg_start; reg<=reg_end;) {
            if (func==0)
                data = sdio_f0_readb(((struct sdio_mmc_card *)phandle->card)->func, reg, &ret);
            else
                data = sdio_readb(((struct sdio_mmc_card *)phandle->card)->func, reg, &ret);
            if (loop == 2)
...
void woal_moal_debug_info(moal_private *priv, moal_handle *handle, u8 flag)
{
    ...
    if (priv) {
        PRINTM(MERROR, "Media state = \"%s\"\n", ((priv->media_connected == MFALSE) ? "Disconnected" : "Connected"));
        PRINTM(MERROR, "carrier %s\n", ((netif_carrier_ok(priv->netdev)) ? "on" : "off"));
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,29)
        for (i = 0; i < (priv->netdev->num_tx_queues); i++) {
            PRINTM(MERROR, "tx queue %d: %s\n", i, ((netif_tx_queue_stopped(netdev_get_tx_queue(priv->netdev, i))) ? "stopped" : "started"));
        }
#else
...
t_void woal_main_work_queue(struct work_struct *work)
{
    ...
    if ((wq_sched_prio != current->rt_priority) || (wq_sched_policy != current->policy)) {
...
moal_handle *woal_add_card(void *card)
{
...
#else
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22)
        handle->nl_sk = netlink_kernel_create(netlink_num, NL_MULTICAST_GROUP, NULL, THIS_MODULE);
#else
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
        handle->nl_sk = netlink_kernel_create(netlink_num, NL_MULTICAST_GROUP, NULL, NULL, THIS_MODULE);
#else
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0)
        handle->nl_sk = netlink_kernel_create(&init_net, netlink_num, NL_MULTICAST_GROUP, NULL, NULL, THIS_MODULE);
#else
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
        handle->nl_sk = netlink_kernel_create(&init_net, netlink_num, THIS_MODULE, &cfg);
#else
    ...
    if(handle->nl_sk == NULL) {
        PRINTM(MERROR, "Could not initialize netlink event passing mechanism!\n");
        goto err_kmalloc;
    }
...
mlan_status woal_remove_card(void *card)
{
    ...
    if (atomic_read(&handle->rx_pending) || atomic_read(&handle->tx_pending) ||atomic_read(&handle->ioctl_pending)) {
        PRINTM(MERROR,"ERR: rx_pending=%d,tx_pending=%d,ioctl_pending=%d\n",
            atomic_read(&handle->rx_pending), atomic_read(&handle->tx_pending),atomic_read(&handle->ioctl_pending));
    }
    unregister_inetaddr_notifier(&woal_notifier);
#if defined(WIFI_DIRECT_SUPPORT)
#if defined(STA_CFG80211) && defined(UAP_CFG80211)
...
static void woal_cleanup_module(void)
{
...
#ifdef STA_CFG80211
            if(IS_STA_CFG80211(cfg80211_wext) && (handle->priv[i]->bss_type == MLAN_BSS_TYPE_STA))
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0) || defined(COMPAT_WIRELESS)
           if (IS_STA_CFG80211(cfg80211_wext) && handle->priv[i]->sched_scanning) {
    ...
    if (mfg_mode != MLAN_INIT_PARA_ENABLED)
#endif
        woal_set_deep_sleep(woal_get_priv(handle,MLAN_BSS_ROLE_ANY),MOAL_CMD_WAIT, MFALSE,0);

#ifdef MFG_CMD_SUPPORT
    if (mfg_mode != MLAN_INIT_PARA_ENABLED)
#endif
        woal_shutdown_fw(woal_get_priv(handle,MLAN_BSS_ROLE_ANY), MOAL_CMD_WAIT);
    }
...
MODULE_PARM_DESC(req_fw_nowait, "0: Use request_firmware API; 1: Use request_firmware_nowait API");
module_param(mac_addr, charp, 0);
MODULE_PARM_DESC(mac_addr, "MAC address");
#ifdef MFG_CMD_SUPPORT
module_param(mfg_mode, int, 0);
MODULE_PARM_DESC(mfg_mode, "0: Download normal firmware; 1: Download MFG firmware");
...
MODULE_PARM_DESC(auto_ds, "0: MLAN default; 1: Enable auto deep sleep; 2: Disable auto deep sleep");
module_param(ps_mode, int, 0);
MODULE_PARM_DESC(ps_mode, "0: MLAN default; 1: Enable IEEE PS mode; 2: Disable IEEE PS mode");
...
MODULE_PARM_DESC(cfg_11d, "0: MLAN default; 1: Enable 802.11d; 2: Disable 802.11d");
#endif
...
MODULE_PARM_DESC(wq_sched_policy, "0: SCHED_NORMAL; 1: SCHED_FIFO; 2: SCHED_RR; 3: SCHED_BATCH; 5: SCHED_IDLE");
