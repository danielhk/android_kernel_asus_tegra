                ...
                struct iw_freq *fwrq, char *extra)
{
        ...
        if(MLAN_STATUS_SUCCESS != woal_change_adhoc_chan(priv,bss->param.bss_chan.channel))
                ret =  -EFAULT;
 ...
 *  @return                     0 --success, otherwise fail
 */
static int woal_set_bss_mode(struct net_device *dev, struct iw_request_info *info,
 ...
 *  @return                     0 --success
 */
static int woal_get_bss_mode(struct net_device *dev, struct iw_request_info *info,
                t_u32 *uwrq, char *extra)
{
        moal_private        *priv = (moal_private *) netdev_priv(dev);
        ENTER();
                ...
                struct iw_param *vwrq, char *extra)
{
    ...
    if (MLAN_STATUS_SUCCESS != woal_set_get_power_mgmt(priv,
        MLAN_ACT_SET, &disabled, vwrq->flags)) {
        ret = -EFAULT;
    }
                ...
                struct iw_point *dwrq, char *extra)
{
        ...
        if(sec->param.encrypt_key.key_len) {
                memcpy(extra, sec->param.encrypt_key.key_material, sec->param.encrypt_key.key_len);
                           ...
                           char *extra)
{
    ...
    if((dwrq->flags & IW_ENCODE_DISABLED) && !ext->key_len){
        ...
        PRINTM(MIOCTL,"Remove key key_index=%d, dwrq->flags=0x%x %02x:%02x:%02x:%02x:%02x:%02x\n",
                key_index, dwrq->flags,
                sec->param.encrypt_key.mac_addr[0],sec->param.encrypt_key.mac_addr[1],
                sec->param.encrypt_key.mac_addr[2],sec->param.encrypt_key.mac_addr[3],
                sec->param.encrypt_key.mac_addr[4],sec->param.encrypt_key.mac_addr[5]);
    }
    else if(ext->key_len <= MAX_WEP_KEY_SIZE){
        /* Set WEP key */
        sec->param.encrypt_key.key_index = key_index;
        sec->param.encrypt_key.key_flags = ext->ext_flags;
        memcpy(sec->param.encrypt_key.key_material, pkey_material, ext->key_len);
        ...
        if(ext->ext_flags & IW_ENCODE_EXT_RX_SEQ_VALID) {
            memcpy(sec->param.encrypt_key.pn, (t_u8*)ext->rx_seq, SEQ_MAX_SIZE);
            DBG_HEXDUMP(MCMD_D,"Rx PN", sec->param.encrypt_key.pn, SEQ_MAX_SIZE);
        }
        if(ext->ext_flags & IW_ENCODE_EXT_TX_SEQ_VALID) {
            memcpy(sec->param.encrypt_key.pn, (t_u8*)ext->tx_seq, SEQ_MAX_SIZE);
            DBG_HEXDUMP(MCMD_D,"Tx PN", sec->param.encrypt_key.pn, SEQ_MAX_SIZE);
        }
        memcpy(sec->param.encrypt_key.key_material, pkey_material, ext->key_len);
        PRINTM(MIOCTL,"set wpa key key_index=%d, key_len=%d key_flags=0x%x %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    if (ext->alg == IW_ENCODE_ALG_AES_CMAC)
        ...
        if(ext->alg == IW_ENCODE_ALG_SMS4) {
            ...
            memcpy(sec->param.encrypt_key.pn, (t_u8*)ext->tx_seq, SEQ_MAX_SIZE);
            memcpy(&sec->param.encrypt_key.pn[SEQ_MAX_SIZE], (t_u8*)ext->rx_seq, SEQ_MAX_SIZE);
 ...
 *  @return             0--success, otherwise fail
 */
static int woal_set_mlme(struct net_device *dev,
                           struct iw_request_info *info,
                           struct iw_point *dwrq,
                           char *extra)
{
    ...
    if((mlme->cmd == IW_MLME_DEAUTH) || (mlme->cmd == IW_MLME_DISASSOC)) {
        ...
        if(MLAN_STATUS_SUCCESS != woal_disconnect(priv,  MOAL_IOCTL_WAIT,(t_u8 *)mlme->addr.sa_data))
              ret = -EFAULT;
    }
    LEAVE();
    return ret;
                ...
                struct iw_param *vwrq, char *extra)
{
    ...
    switch (vwrq->flags & IW_AUTH_INDEX) {
        ...
        case IW_AUTH_WPA_ENABLED:
            if(MLAN_STATUS_SUCCESS != woal_set_wpa_enable(priv,MOAL_IOCTL_WAIT,vwrq->value))
                ret =  -EFAULT;
            break;
#define IW_AUTH_WAPI_ENABLED    0x20
        case IW_AUTH_WAPI_ENABLED:
            if(MLAN_STATUS_SUCCESS != woal_set_wapi_enable(priv,MOAL_IOCTL_WAIT,vwrq->value))
                ret =  -EFAULT;
            break;
                ...
                struct iw_param *vwrq, char *extra)
{
        ...
        switch (vwrq->flags & IW_AUTH_INDEX) {
                case IW_AUTH_CIPHER_PAIRWISE:
                case IW_AUTH_CIPHER_GROUP:
                        if(MLAN_STATUS_SUCCESS != woal_get_encrypt_mode(priv,MOAL_IOCTL_WAIT,&encrypt_mode))
                                ret =  -EFAULT;
                        else {
                ...
                case IW_AUTH_80211_AUTH_ALG:
                        ...
                        break;
                case IW_AUTH_WPA_ENABLED:
                        if(MLAN_STATUS_SUCCESS !=  woal_get_wpa_enable(priv,MOAL_IOCTL_WAIT,&wpa_enable))
                                ret =  -EFAULT;
                        else
                          ...
                          struct iw_point *dwrq, char *extra)
{
        ...
        for (i=0; i < MIN(range->num_bitrates,IW_MAX_BITRATES) && rates.rates[i]; i++) {
                ...
                struct iw_point* dwrq, char *extra)
{
    ...
    if(strncmp(buf,"RSSILOW-THRESHOLD",strlen("RSSILOW-THRESHOLD")) == 0) {
        ...
        if (bss_info.media_connected){
            ...
            len = sprintf(buf,"%s rssi %d\n",bss_info.ssid.ssid,signal.bcn_rssi_avg) + 1;
        }
        else {
            len = sprintf(buf,"OK\n") + 1;
        }
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_get_data_rate(priv,MLAN_ACT_GET,&rate)){
            ret = - EFAULT;
            goto done;
        }
        ...
        len = sprintf(buf,"Macaddr = %02X:%02X:%02X:%02X:%02X:%02X\n",priv->current_addr[0],
                priv->current_addr[1],priv->current_addr[2],priv->current_addr[3],
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_scan_type(priv,MLAN_SCAN_TYPE_ACTIVE)){
            ret = - EFAULT;
            goto done;
        }
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_scan_type(priv,MLAN_SCAN_TYPE_PASSIVE)){
            ret = - EFAULT;
            goto done;
        }
        ...
        memcpy(country_code,buf + strlen("COUNTRY") + 1,  strlen(buf) - strlen("COUNTRY")-1);
    ...
    if ((dwrq->flags & IW_SCAN_THIS_ESSID) &&
        ...
        if (req->essid_len <= MLAN_MAX_SSID_LENGTH) {
            ...
            memcpy(req_ssid.ssid,
                   (t_u8 *)req->essid,
                   req->essid_len);
            if (MLAN_STATUS_SUCCESS != woal_request_scan(priv,  MOAL_NO_WAIT, &req_ssid)) {
                ret = -EFAULT;
                goto done;
            }
        }
                          ...
                          struct iw_point* dwrq, char* extra)
{
    ...
    if (!req_ssid.ssid_len) {
        ...
        /* Set the SSID */
        memcpy(req_ssid.ssid, extra, MIN(req_ssid.ssid_len, MLAN_MAX_SSID_LENGTH));
                  ...
                  struct iw_point* dwrq, char* extra)
{
        ...
        for (i = 0; i < scan_resp.num_in_scan_table; i++) {
                ...
                current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe,(char*)scan_table[i].ssid.ssid);
                ...
                if ((bss_info.bss_mode == MLAN_BSS_MODE_IBSS) &&
                        ...
                        if(MLAN_STATUS_SUCCESS != woal_get_signal_info(priv, MOAL_IOCTL_WAIT, &rssi)) {
                                ret = -EFAULT;
                                break;
                        }
                        iwe.u.qual.level = rssi.data_rssi_avg;
                }
                iwe.u.qual.qual = woal_rssi_to_quality((t_s16)(iwe.u.qual.level-0x100));
                ...
                for (j = 0; j < sizeof(scan_table[i]. supported_rates); j++){
                        ...
                        iwe.u.bitrate.value = (scan_table[i].supported_rates[j] & 0x7f) * 500000;
                ...
                if (pbeacon) {
                    ...
                    while ((unsigned int)beacon_size >= sizeof(IEEEtypes_Header_t)) {
                        ...
                        if ((unsigned int)beacon_size < (unsigned int)element_len + sizeof(IEEEtypes_Header_t)) {
                        ...
                        switch (element_id) {
                            ...
                            case WAPI_IE:
                                ...
                                memset(buf, 0, buf_size);
                                ptr = buf;
                                memcpy(buf, praw_data, element_len + sizeof(IEEEtypes_Header_t));
                                iwe.cmd = IWEVGENIE;
