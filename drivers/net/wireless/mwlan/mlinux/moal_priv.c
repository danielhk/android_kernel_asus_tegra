...
static int woal_deep_sleep_ioctl(moal_private *priv, struct iwreq *wrq)
{
	...
	if (wrq->u.data.length == 1 || wrq->u.data.length == 2) {
		if (copy_from_user(&data, wrq->u.data.pointer, wrq->u.data.length*sizeof(int))) {
			PRINTM(MERROR, "Copy from user failed\n");
			LEAVE();
			return -EFAULT;
		}
	...
	/* Copy the Deep Sleep setting to user */
        if (copy_to_user(wrq->u.data.pointer, data, wrq->u.data.length * sizeof(int))) {
		PRINTM(MERROR, "Copy to user failed\n");
		LEAVE();
		return -EINVAL;
	}
...
static int woal_11n_htcap_cfg(moal_private *priv, struct iwreq *wrq)
{
    ...
    if (data_length == 0) {
        ...
        if (copy_from_user(data, wrq->u.data.pointer, data_length * sizeof(int))) {
            PRINTM(MERROR, "Copy from user failed\n");
            ret = -EFAULT;
            goto done;
        }
...
static int woal_txbuf_cfg(moal_private *priv, struct iwreq *wrq)
{
	...
	if (wrq->u.data.length == 0) {
        ...
        ret = -EINVAL;
        PRINTM(MERROR, "Don't support set Tx buffer size after driver loaded!\n");
        goto done;
    }
    if(MLAN_STATUS_SUCCESS != woal_request_ioctl(priv,req,MOAL_IOCTL_WAIT)) {
        ret = -EFAULT;
 ...
 *  @return             0 --success, otherwise fail
 */
static int woal_hs_cfg(moal_private *priv, struct iwreq *wrq, BOOLEAN invoke_hostcmd)
{
	int data[3];
	int ret = 0;
	...
	if (data_length && (data[0] != HOST_SLEEP_CFG_CANCEL || invoke_hostcmd == MFALSE)) {
...
static int woal_ecl_sys_clock(moal_private *priv, struct iwreq *wrq)
{
	...
	if (req->action == MLAN_ACT_GET) {
		...
		for (i = 0; i < data_length; i++) {
			data[i + wrq->u.data.length] = (int) cfg->param.sys_clock.sys_clk[i];
		}
		wrq->u.data.length += data_length;
...
static int woal_band_cfg(moal_private *priv, struct iwreq *wrq)
{
	...
	if (wrq->u.data.length == 0) {
		...
		data[2] = radio_cfg->param.band_cfg.adhoc_channel;     /* Adhoc Channel */
		wrq->u.data.length = 3;
...
static int woal_reg_read_write(moal_private *priv, struct iwreq *wrq)
{
	...
	if (req->action == MLAN_ACT_GET) {
		if (copy_to_user(wrq->u.data.pointer, &reg->param.reg_rw.value, sizeof(int))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
			goto done;
		}
...
static int woal_read_eeprom(moal_private *priv, struct iwreq *wrq)
{
	...
	if (req->action == MLAN_ACT_GET) {
		...
		if (copy_to_user(wrq->u.data.pointer, reg->param.rd_eeprom.value, MIN(wrq->u.data.length, MAX_EEPROM_DATA))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
			goto done;
		}
...
static int woal_mem_read_write(moal_private *priv, struct iwreq *wrq)
{
	...
	if (req->action == MLAN_ACT_GET) {
		if (copy_to_user(wrq->u.data.pointer, &reg_mem->param.mem_rw.value, sizeof(int))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
			goto done;
		}
...
static int woal_deauth(moal_private *priv, struct iwreq *wrq)
{
    ...
    if (wrq->u.data.length) {
        ...
        if(MLAN_STATUS_SUCCESS != woal_disconnect(priv, MOAL_IOCTL_WAIT,(t_u8 *)saddr.sa_data)) {
            ret = -EFAULT;
            goto done;
        }
    } else {
...
static int woal_tx_power_cfg(moal_private* priv, struct iwreq* wrq)
{
	...
	if(!user_data_len)
		...
		memcpy((t_u8*)&pcfg->param.power_ext.power_data,(t_u8*)data,sizeof(data));
	}
	if(MLAN_STATUS_SUCCESS != woal_request_ioctl(priv,req,MOAL_IOCTL_WAIT)) {
		ret =  -EFAULT;
		goto done;
	...
	if(!user_data_len) {
		/* GET operation */
		if (copy_to_user(wrq->u.data.pointer,(t_u8*)&pcfg->param.power_ext.power_data,
...
static int woal_get_txrx_rate(moal_private* priv, struct iwreq* wrq)
{
	...
	if (copy_to_user(wrq->u.data.pointer,(t_u8*)&rate->param.data_rate, sizeof(int) * 2)) {
		ret = -EFAULT;
		goto done;
	}
	wrq->u.data.length = 2;
...
static int woal_beacon_interval(moal_private* priv, struct iwreq* wrq)
{
	...
	if(wrq->u.data.length) {
		...
		if((bcn < MLAN_MIN_BEACON_INTERVAL) || (bcn > MLAN_MAX_BEACON_INTERVAL)) {
			ret = -EINVAL;
			goto done;
		}
	}
	...
	if (copy_to_user(wrq->u.data.pointer,(t_u8*)&bss->param.bcn_interval, sizeof(int))) {
		ret = -EFAULT;
		goto done;
	}
	wrq->u.data.length = 1;
 ...
 *  @return         0 --success, otherwise fail
 */
static int woal_atim_window(moal_private* priv, struct iwreq* wrq)
{
	int ret = 0;
	mlan_ds_bss         *bss = NULL;
	mlan_ioctl_req	    *req = NULL;
	...
	if (copy_to_user(wrq->u.data.pointer,(t_u8*)&bss->param.atim_window, sizeof(int))) {
		ret = -EFAULT;
		goto done;
	}
	wrq->u.data.length = 1;
...
static int woal_set_get_regioncode(moal_private* priv, struct iwreq* wrq)
{
	...
	if(!wrq->u.data.length) {
		wrq->u.data.length = 1;
		if (copy_to_user(wrq->u.data.pointer, &cfg->param.region_code, sizeof(int)))
			ret = -EFAULT;
	}
done:
	if(req)
...
static int woal_set_get_radio(moal_private* priv, struct iwreq* wrq)
{
	...
	if(wrq->u.data.length) {
		...
		woal_get_bss_info(priv, MOAL_IOCTL_WAIT, &bss_info);
		wrq->u.data.length = 1;
		if (copy_to_user(wrq->u.data.pointer, &bss_info.radio_on, sizeof(bss_info.radio_on))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
		}
	}
 ...
 *  @return         0 --success, otherwise fail
 */
static int woal_set_get_11h_local_pwr_constraint(moal_private *priv, struct iwreq *wrq)
{
	int ret = 0, data = 0;
	mlan_ioctl_req *req = NULL;
...
static int woal_wmm_enable_ioctl(moal_private *priv, struct iwreq *wrq)
{
	...
	if (wrq->u.data.pointer) {
		if (copy_to_user(wrq->u.data.pointer, &wmm->param.wmm_enable, sizeof(wmm->param.wmm_enable))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
			goto done;
		}
...
static int woal_11d_enable_ioctl(moal_private *priv, struct iwreq *wrq)
{
	...
	if (wrq->u.data.pointer) {
		if (copy_to_user(wrq->u.data.pointer, &pcfg_11d->param.enable_11d, sizeof(pcfg_11d->param.enable_11d))) {
			PRINTM(MERROR, "Copy to user failed\n");
			ret = -EFAULT;
			goto done;
		}
...
static int woal_passphrase(moal_private *priv, struct iwreq *wrq)
{
	...
	if(action == 0) {
		...
		if(sec->param.passphrase.ssid.ssid_len) {
			...
			memcpy(buf +len, sec->param.passphrase.ssid.ssid,sec->param.passphrase.ssid.ssid_len);
		...
		if(sec->param.passphrase.psk_type == MLAN_PSK_PMK) {
			...
			for (i=0; i<MLAN_MAX_KEY_LENGTH; ++i)
				len += sprintf(buf+len,"%02x",sec->param.passphrase.psk.pmk.pmk[i]);
			len += sprintf(buf+len, "\n");
		}
...
static int woal_get_esupp_mode(moal_private* priv, struct iwreq* wrq)
{
	...
	if (copy_to_user(wrq->u.data.pointer,(t_u8*)&sec->param.esupp_mode, sizeof(int) * 3)) {
		ret = -EFAULT;
		goto done;
	}
	wrq->u.data.length = 3;
 ...
 *  @return	    0 --success, otherwise fail
 */
static int woal_adhoc_aes_ioctl(moal_private *priv, struct iwreq *wrq)
{
    static char buf[256];
    int ret = 0, action = -1;
    unsigned int i;
    ...
    if (wrq->u.data.length) {
        ...
        if (wrq->u.data.length == 1) {
            ...
            memcpy(key_hex, sec->param.encrypt_key.key_material, sizeof(key_hex));
            ...
            if (action == 1) {
                ...
                sec->param.encrypt_key.key_len = AES_KEY_LEN;
                sec->param.encrypt_key.key_index = MLAN_KEY_INDEX_UNICAST;
                sec->param.encrypt_key.key_flags = KEY_FLAG_SET_TX_KEY | KEY_FLAG_GROUP_KEY;
                memcpy(sec->param.encrypt_key.mac_addr,(u8*)bcast_addr,ETH_ALEN);
...
static int woal_arp_filter(moal_private * priv, struct iwreq *wrq)
{
	...
	if (copy_from_user(misc->param.gen_ie.ie_data, wrq->u.data.pointer, wrq->u.data.length)) {
		PRINTM(MERROR, "copy from user failed\n");
		ret = -EFAULT;
		goto done;
	}
...
static int woal_tx_bf_cfg_ioctl(moal_private * priv, struct iwreq *wrq)
{
    ...
    if (action == MLAN_ACT_GET) {
        ...
        switch (bf_action) {
        case BF_GLOBAL_CONFIGURATION:
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->bf_enbl);
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->sounding_enbl);
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->fb_type);
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->snr_threshold);
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->sounding_interval);
            data_length += sprintf(buf + data_length, "%d ", (int)bf_global->bf_mode);
            break;
        case TRIGGER_SOUNDING_FOR_PEER:
        ...
        case SET_GET_BF_PERIODICITY:
            ...
            data_length += sprintf(buf + data_length, "%c", ' ');
            data_length += sprintf(buf + data_length, "%d", bf_periodicity->interval);
            break;
        case TX_BF_FOR_PEER_ENBL:
            for (i = 0; i < bf_cfg.no_of_peers; i ++) {
                ...
                data_length += sprintf(buf + data_length, "%c", ' ');
                data_length += sprintf(buf + data_length, "%d;", tx_bf_peer->bf_enbl);
                data_length += sprintf(buf + data_length, "%d;", tx_bf_peer->sounding_enbl);
                data_length += sprintf(buf + data_length, "%d ", tx_bf_peer->fb_type);
                tx_bf_peer ++;
            }
            break;
        case SET_SNR_THR_PEER:
 ...
 */
static mlan_status woal_get_scan_table_ioctl(moal_private * priv, struct iwreq *wrq)
{
    int             ret = 0;
    mlan_ioctl_req  *req = NULL;
 ...
 *  @return         0 -- success, otherwise fail
 */
static int woal_set_user_scan_ext_ioctl(moal_private *priv,
                                            struct iwreq *wrq)
{
    int              ret = 0;
    ...
    if (copy_from_user(&scan_req, wrq->u.data.pointer, MIN(wrq->u.data.length,sizeof(scan_req)))) {
        PRINTM(MINFO, "Copy from user failed\n");
        LEAVE();
        return -EFAULT;
    }
...
static int woal_cmd53rdwr_ioctl(moal_private *priv, struct iwreq *wrq)
{
    ...
    if (!rw) {
        sdio_claim_host(((struct sdio_mmc_card *)priv->phandle->card)->func);
        if (sdio_readsb(((struct sdio_mmc_card *)priv->phandle->card)->func, data, reg, total_len))
        ...
        sdio_claim_host(((struct sdio_mmc_card *)priv->phandle->card)->func);
        if (sdio_writesb(((struct sdio_mmc_card *)priv->phandle->card)->func, reg, data, total_len))
...
static int woal_wmm_queue_stats_ioctl(moal_private *priv, struct iwreq* wrq)
{
    ...
    if (wrq->u.data.length) {
        ...
        if (copy_to_user(wrq->u.data.pointer, &qstats_ioctl, wrq->u.data.length)) {
...
static int woal_auth_type(moal_private *priv, struct iwreq *wrq)
{
    ...
    ENTER();
    if (wrq->u.data.length == 0) {
        if(MLAN_STATUS_SUCCESS != woal_get_auth_mode(priv,MOAL_IOCTL_WAIT,&auth_mode)) {
            ret =  -EFAULT;
            goto done;
        }
            auth_type = auth_mode;
        ...
        if(((auth_type < MLAN_AUTH_MODE_OPEN) || (auth_type > MLAN_AUTH_MODE_SHARED))
...
int woal_wext_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    case WOAL_SET_GET_SIXTEEN_INT:
        ...
        case WOAL_11N_AMSDU_AGGR_CTRL:
            ret = woal_11n_amsdu_aggr_ctrl(priv,
                                           wrq);
            break;
 ...
 *  @return             MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING -- success, otherwise fail
 */
mlan_status woal_get_stats_info(moal_private * priv, t_u8 wait_option, mlan_ds_get_stats * stats)
{
        int                     ret = 0;
 ...
 *  @return              MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING -- success, otherwise fail
 */
mlan_status woal_get_data_rates(moal_private * priv, t_u8 wait_option, moal_802_11_rates * m_rates)
{
        int             ret = 0;
        mlan_ds_rate    * rate = NULL;
        mlan_ioctl_req  *req = NULL;
        ...
        if(status == MLAN_STATUS_SUCCESS) {
                if(m_rates)
                        m_rates->num_of_rates = woal_copy_rates(m_rates->rates,m_rates->num_of_rates,
 ...
 *  @return                MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING -- success, otherwise fail
 */
mlan_status woal_get_channel_list(moal_private * priv, t_u8 wait_option, mlan_chan_list * chan_list)
{
        int             ret = 0;
        mlan_ds_bss     * bss = NULL;
        mlan_ioctl_req  *req = NULL;
