                        ...
                        OUT t_u8 ** ppbuf)
{
	...
	t_u32 mem_flag = (in_interrupt() || irqs_disabled() || !write_can_lock(&dev_base_lock)) ?
                        ...
                        OUT t_u8 ** ppbuf)
{
    ...
    if (!(*ppbuf = vmalloc(size))) {
        PRINTM(MERROR, "%s: vmalloc (%d bytes) failed!", __FUNCTION__, (int)size);
        return MLAN_STATUS_FAILURE;
    }
...
t_void *moal_memmove(IN t_void * pmoal_handle,
                     IN t_void *pdest, IN const t_void *psrc,
                     IN t_u32 num)
{
    t_void *p = pdest;
    ...
    )
{
	...
	if(!(timer = (moal_drv_timer *)kmalloc(sizeof(moal_drv_timer), GFP_KERNEL)))
		return MLAN_STATUS_FAILURE;
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
mlan_status moal_init_fw_complete(IN t_void *pmoal_handle, IN mlan_status status)
{
	moal_handle * handle = (moal_handle *)pmoal_handle;
	ENTER();
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
mlan_status moal_shutdown_fw_complete(IN t_void *pmoal_handle, IN mlan_status status)
{
	moal_handle * handle = (moal_handle *)pmoal_handle;
	ENTER();
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
mlan_status moal_ioctl_complete (
		IN t_void  *pmoal_handle,
		...
		)
{
    ...
    PRINTM(MIOCTL, "IOCTL completed: %p id=0x%x sub_id=0x%x, action=%d,  status=%d, status_code=0x%x\n",
    ...
    if(wait) {
        ...
        wait->status = status;
        if((status != MLAN_STATUS_SUCCESS) && (pioctl_req->status_code == MLAN_ERROR_CMD_TIMEOUT)) {
        ...
        if(priv && (status == MLAN_STATUS_SUCCESS) && (pioctl_req->action == MLAN_ACT_GET))
            woal_process_ioctl_resp(priv,pioctl_req);
        if(status != MLAN_STATUS_SUCCESS)
            PRINTM(MERROR, "IOCTL failed: id=0x%x, action=%d, status_code=0x%x\n",pioctl_req->req_id,
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status moal_alloc_mlan_buffer(
	IN t_void *pmoal_handle,
	IN t_u32 size,
	OUT pmlan_buffer *pmbuf)
{
	if(NULL == (*pmbuf = woal_alloc_mlan_buffer((moal_handle*)pmoal_handle, size)))
		return MLAN_STATUS_FAILURE;
	return MLAN_STATUS_SUCCESS;
}
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status moal_free_mlan_buffer(
	IN t_void *pmoal_handle,
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
mlan_status moal_send_packet_complete(
	IN t_void *pmoal_handle,
	IN pmlan_buffer pmbuf,
	IN mlan_status status)
{
                ...
                if(atomic_dec_return(&handle->tx_pending) <  LOW_TX_PENDING) {
                    ...
                    for (i = 0; i < handle->priv_num; i++) {
                        ...
                        if ((GET_BSS_ROLE(handle->priv[i]) == MLAN_BSS_ROLE_STA) &&
                            (handle->priv[i]->media_connected || priv->is_adhoc_link_sensed)) {
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status moal_write_data_sync (
	IN t_void           *pmoal_handle,
	IN pmlan_buffer     pmbuf,
	IN t_u32            port,
	IN t_u32	    timeout
...
{
	return woal_write_data_sync((moal_handle *)pmoal_handle, pmbuf, port, timeout);
}
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status moal_read_data_sync (
	IN t_void		*pmoal_handle,
...
{
	return woal_read_data_sync((moal_handle *)pmoal_handle, pmbuf, port, timeout);
}
    ...
    if(pmbuf) {
        ...
        if(priv) {
            if(skb) {
                ...
                memcpy(skb->data,(t_u8*)(pmbuf->pbuf + pmbuf->data_offset),pmbuf->data_len);
                skb_put(skb,pmbuf->data_len);
            }
            skb->dev = priv->netdev;
...
)
{
    ...
    switch (pmevent->event_id) {
...
#endif
            woal_broadcast_event(priv, CUS_EVT_ADHOC_LINK_SENSED,strlen(CUS_EVT_ADHOC_LINK_SENSED));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_ADHOC_LINK_LOST,strlen(CUS_EVT_ADHOC_LINK_LOST));
            break;
	    ...
	    memmove(pmevent->event_buf + custom_len, pmevent->event_buf,pmevent->event_len);
        ...
        case MLAN_EVENT_ID_DRV_SCAN_REPORT:
            ...
            if(priv->report_scan_result) {
               ...
               if (IS_STA_CFG80211(cfg80211_wext)) {
                ...
                woal_broadcast_event(priv, (t_u8 *)&pmevent->event_id, sizeof(mlan_event_id));
        ...
        case MLAN_EVENT_ID_DRV_OBSS_SCAN_PARAM:
            ...
            woal_broadcast_event(priv,pmevent->event_buf,pmevent->event_len + strlen(CUS_EVT_OBSS_SCAN_PARAM));
        ...
        case MLAN_EVENT_ID_FW_BW_CHANGED:
            ...
            woal_broadcast_event(priv,pmevent->event_buf,pmevent->event_len + strlen(CUS_EVT_BW_CHANGED));
        ...
        case MLAN_EVENT_ID_FW_MIC_ERR_UNI:
            ...
            if (IS_STA_CFG80211(cfg80211_wext)) {
                cfg80211_michael_mic_failure(priv->netdev, priv->cfg_bssid,
                    NL80211_KEYTYPE_PAIRWISE, -1, NULL, GFP_KERNEL);
            }
#endif
            woal_broadcast_event(priv, CUS_EVT_MLME_MIC_ERR_UNI,strlen(CUS_EVT_MLME_MIC_ERR_UNI));
            break;
        case MLAN_EVENT_ID_FW_MIC_ERR_MUL:
            ...
            if (IS_STA_CFG80211(cfg80211_wext)) {
                cfg80211_michael_mic_failure(priv->netdev, priv->cfg_bssid,
                    NL80211_KEYTYPE_GROUP, -1, NULL, GFP_KERNEL);
            }
#endif
            woal_broadcast_event(priv, CUS_EVT_MLME_MIC_ERR_MUL,strlen(CUS_EVT_MLME_MIC_ERR_MUL));
            break;
        case MLAN_EVENT_ID_FW_BCN_RSSI_LOW:
            ...
            if (IS_STA_CFG80211(cfg80211_wext)) {
                ...
                cfg80211_cqm_rssi_notify(priv->netdev,
                    NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW,GFP_KERNEL);
                priv->last_event |= EVENT_BCN_RSSI_LOW;
#endif
                woal_config_bgscan_and_rssi(priv,MTRUE);
            }
#endif
            woal_broadcast_event(priv, CUS_EVT_BEACON_RSSI_LOW,strlen(CUS_EVT_BEACON_RSSI_LOW));
            break;
...
#endif
                    woal_set_rssi_threshold(priv, MLAN_EVENT_ID_FW_BCN_RSSI_HIGH,MOAL_NO_WAIT);
                }
            }
#endif
            woal_broadcast_event(priv, CUS_EVT_BEACON_RSSI_HIGH,strlen(CUS_EVT_BEACON_RSSI_HIGH));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_BEACON_SNR_LOW,strlen(CUS_EVT_BEACON_SNR_LOW));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_BEACON_SNR_HIGH,strlen(CUS_EVT_BEACON_SNR_HIGH));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_DATA_RSSI_LOW,strlen(CUS_EVT_DATA_RSSI_LOW));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_DATA_SNR_LOW,strlen(CUS_EVT_DATA_SNR_LOW));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_DATA_RSSI_HIGH,strlen(CUS_EVT_DATA_RSSI_HIGH));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_DATA_SNR_HIGH,strlen(CUS_EVT_DATA_SNR_HIGH));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_LINK_QUALITY,strlen(CUS_EVT_LINK_QUALITY));
            break;
...
#endif
            woal_broadcast_event(priv, CUS_EVT_PORT_RELEASE,strlen(CUS_EVT_PORT_RELEASE));
            break;
        case  MLAN_EVENT_ID_FW_PRE_BCN_LOST:
            ...
            if (IS_STA_CFG80211(cfg80211_wext)) {
                ...
                bss = cfg80211_get_bss(priv->wdev->wiphy, NULL, priv->cfg_bssid, NULL, 0,
...
#endif
#endif
            woal_broadcast_event(priv, CUS_EVT_PRE_BEACON_LOST,strlen(CUS_EVT_PRE_BEACON_LOST));
            break;
...
#endif
            woal_broadcast_event(priv, WMM_CONFIG_CHANGE_INDICATION,strlen(WMM_CONFIG_CHANGE_INDICATION));
            break;
	...
	case MLAN_EVENT_ID_DRV_REPORT_STRING:
	    PRINTM(MINFO, "Report string %s\n", pmevent->event_buf);
            ...
            if (IS_STA_WEXT(cfg80211_wext))
                woal_send_iwevcustom_event(priv, pmevent->event_buf);
#endif
            woal_broadcast_event(priv, pmevent->event_buf, strlen(pmevent->event_buf));
	    break;
        case MLAN_EVENT_ID_FW_WEP_ICV_ERR:
            DBG_HEXDUMP(MCMD_D, "WEP ICV error", pmevent->event_buf, pmevent->event_len);
#ifdef STA_WEXT
            if (IS_STA_WEXT(cfg80211_wext))
                woal_send_iwevcustom_event(priv, CUS_EVT_WEP_ICV_ERR);
#endif
            woal_broadcast_event(priv, CUS_EVT_WEP_ICV_ERR,strlen(CUS_EVT_WEP_ICV_ERR));
            break;
	  ...
	  case MLAN_EVENT_ID_DRV_BGSCAN_RESULT:
...
#endif
            woal_broadcast_event(priv, CUS_EVT_CHANNEL_SWITCH_ANN, strlen(CUS_EVT_CHANNEL_SWITCH_ANN));
            break;
#endif /* STA_SUPPORT */
...
#ifdef STA_SUPPORT
#ifdef STA_WEXT
            if (IS_STA_WEXT(cfg80211_wext) && (pmpriv = woal_get_priv(
                            (moal_handle *)pmoal_handle, MLAN_BSS_ROLE_STA)))
                woal_send_iwevcustom_event(pmpriv, CUS_EVT_HS_WAKEUP);
#endif /* STA_WEXT */
            if ((pmpriv = woal_get_priv((moal_handle *)pmoal_handle, MLAN_BSS_ROLE_STA)))
                woal_broadcast_event(pmpriv, CUS_EVT_HS_WAKEUP, strlen(CUS_EVT_HS_WAKEUP));
#endif /*STA_SUPPORT */
#ifdef UAP_SUPPORT
            if ((pmpriv = woal_get_priv((moal_handle *)pmoal_handle, MLAN_BSS_ROLE_UAP))){
                pmevent->event_id = UAP_EVENT_ID_HS_WAKEUP;
                woal_broadcast_event(pmpriv,(t_u8 *)&pmevent->event_id,sizeof(t_u32));
            }
#endif /* UAP_SUPPORT */
            break;
...
#endif /* STA_WEXT */
            if ((pmpriv = woal_get_priv((moal_handle *)pmoal_handle, MLAN_BSS_ROLE_STA)))
                woal_broadcast_event(pmpriv, CUS_EVT_HS_ACTIVATED, strlen(CUS_EVT_HS_ACTIVATED));
#endif /* STA_SUPPORT */
...
#endif /* STA_WEXT */
            if ((pmpriv = woal_get_priv((moal_handle *)pmoal_handle, MLAN_BSS_ROLE_STA)))
                woal_broadcast_event(pmpriv, CUS_EVT_HS_DEACTIVATED, strlen(CUS_EVT_HS_DEACTIVATED));
#endif /* STA_SUPPORT */
#if defined(UAP_SUPPORT)
            if ((pmpriv = woal_get_priv((moal_handle *)pmoal_handle, MLAN_BSS_ROLE_UAP))){
        ...
        case MLAN_EVENT_ID_UAP_FW_STA_CONNECT:
            ...
            woal_broadcast_event(priv,pmevent->event_buf,pmevent->event_len + strlen(CUS_EVT_STA_CONNECTED));
#ifdef UAP_WEXT
            if (IS_UAP_WEXT(cfg80211_wext)) {
                memset(&wrqu, 0, sizeof(union iwreq_data));
                wrqu.data.pointer = pmevent->event_buf;
                if((pmevent->event_len + strlen(CUS_EVT_STA_CONNECTED) + 1) > IW_CUSTOM_MAX)
                    wrqu.data.length  = ETH_ALEN + strlen(CUS_EVT_STA_CONNECTED) + 1;
                else
                    wrqu.data.length  = pmevent->event_len + strlen(CUS_EVT_STA_CONNECTED) + 1;
                wireless_send_event(priv->netdev, IWEVCUSTOM, &wrqu, pmevent->event_buf);
            }
#endif /* UAP_WEXT */
            break;
        case MLAN_EVENT_ID_UAP_FW_STA_DISCONNECT:
            ...
            woal_broadcast_event(priv,pmevent->event_buf,pmevent->event_len + strlen(CUS_EVT_STA_DISCONNECTED));

#ifdef UAP_WEXT
            if (IS_UAP_WEXT(cfg80211_wext)) {
                memset(&wrqu, 0, sizeof(union iwreq_data));
                wrqu.data.pointer = pmevent->event_buf;
                wrqu.data.length  = pmevent->event_len + strlen(CUS_EVT_STA_DISCONNECTED) + 1;
                wireless_send_event(priv->netdev, IWEVCUSTOM, &wrqu, pmevent->event_buf);
            }
#endif /* UAP_WEXT */
            break;
...
#endif /* DEBUG_LEVEL2 */
		{
			if(level & MERROR) DBG_HEXDUMP(MERROR, (char*) pformat, buf, len);
			if(level & MCMD_D) DBG_HEXDUMP(MCMD_D, (char*) pformat, buf, len);
			if(level & MDAT_D) DBG_HEXDUMP(MDAT_D, (char*) pformat, buf, len);
			if(level & MIF_D)  DBG_HEXDUMP(MIF_D,  (char*) pformat, buf, len);
			if(level & MFW_D)  DBG_HEXDUMP(MFW_D,  (char*) pformat, buf, len);
			if(level & MEVT_D) DBG_HEXDUMP(MEVT_D, (char*) pformat, buf, len);
		}
	} else {
		if (drvdbg & level) {
			va_start(args, pformat);
