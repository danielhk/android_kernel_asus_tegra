 ...
 * @return                MTRUE/MFALSE
 */
static t_u8 woal_check_rsn_ie(IEEEtypes_Rsn_t* rsn_ie, mlan_uap_bss_param* sys_config)
{
	int left = 0;
	int count = 0;
	int i = 0;
	...
	if(left < sizeof(wpa_suite_auth_key_mgmt_t))
		return MFALSE;
	key_mgmt = (wpa_suite_auth_key_mgmt_t*)((u8*)rsn_ie + sizeof(IEEEtypes_Rsn_t) +
 ...
 * @return                MTRUE/MFALSE
 */
static t_u8 woal_check_wpa_ie(IEEEtypes_Wpa_t * wpa_ie, mlan_uap_bss_param* sys_config)
{
	int left = 0;
	int count = 0;
	int i = 0;
	...
	if(left < sizeof(wpa_suite_auth_key_mgmt_t))
		return MFALSE;
	key_mgmt = (wpa_suite_auth_key_mgmt_t*)((u8*)wpa_ie + sizeof(IEEEtypes_Wpa_t) +
			(count-1) * sizeof(wpa_suite));
	count = le16_to_cpu(key_mgmt->count);
	if(left < (sizeof(wpa_suite_auth_key_mgmt_t) + (count-1) * sizeof(wpa_suite)))
		return MFALSE;
 ...
 * @return                MTRUE/MFALSE
 */
static t_u8 woal_find_wpa_ies(t_u8 *ie, int len, mlan_uap_bss_param* sys_config)
{
	int bytes_left = len;
	...
	while (bytes_left >= 2) {
		...
		switch (element_id) {
			case RSN_IE:
				wpa2 = woal_check_rsn_ie((IEEEtypes_Rsn_t*) pcurrent_ptr, sys_config);
				break;
			case VENDOR_SPECIFIC_221:
				pvendor_ie = (IEEEtypes_VendorSpecific_t*)pcurrent_ptr;
				if (!memcmp(pvendor_ie->vend_hdr.oui, wpa_oui, sizeof(wpa_oui)))
					wpa = woal_check_wpa_ie((IEEEtypes_Wpa_t *)pcurrent_ptr, sys_config);
				break;
			default:
				break;
		}
 ...
 */
static t_u8 woal_get_second_channel_offset(int chan)
{
...
#endif
{
	...
	t_u16  capab_info = 0;
#endif
	t_u8  Rates_BG[13] = { 0x82, 0x84, 0x8b, 0x96, 0x0c, 0x12, 0x18, 0x24, 0x30, 0x48,
                           0x60, 0x6c, 0 };
	...
	memcpy(sys_config.ssid.ssid,params->ssid,MIN(MLAN_MAX_SSID_LENGTH,params->ssid_len));
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
		woal_find_wpa_ies((t_u8 *)params->beacon.tail, (int)params->beacon.tail_len, &sys_config);
#else
	...
	switch (params->crypto.cipher_group) {
		case WLAN_CIPHER_SUITE_WEP40:
		case WLAN_CIPHER_SUITE_WEP104:
			if((priv->cipher == WLAN_CIPHER_SUITE_WEP40) || (priv->cipher == WLAN_CIPHER_SUITE_WEP104)){
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
			if(MFALSE == woal_find_wpa_ies((t_u8 *)params->beacon.tail, (int)params->beacon.tail_len, &sys_config))
#else
			if(MFALSE == woal_find_wpa_ies(params->tail,params->tail_len, &sys_config))
#endif
			{
	...
	if (MLAN_STATUS_SUCCESS != woal_set_get_sys_config(priv,
				MLAN_ACT_SET, MOAL_IOCTL_WAIT,&sys_config)) {
		ret = -EFAULT;
		goto done;
	}
done:
...
#define MAX_WMM_QUEUE   4
    ...
    if(!(dev = alloc_netdev_mq(sizeof(moal_private), name, woal_virt_if_setup, MAX_WMM_QUEUE))) {
#else
                    ...
                    struct net_device **new_dev)
{
    ...
    ENTER();
    ASSERT_RTNL();
    priv = (moal_private *)woal_get_priv_bss_type(handle, MLAN_BSS_TYPE_WIFIDIRECT);
    ...
    new_priv = woal_alloc_virt_interface(handle, handle->priv_num, MLAN_BSS_TYPE_WIFIDIRECT, name);
    if (!new_priv) {
        ...
        if(priv->phandle->remain_on_channel){
            ...
            if(remain_priv){
                if(woal_cfg80211_remain_on_channel_cfg(remain_priv,	MOAL_IOCTL_WAIT,MTRUE,&channel_status, NULL, 0, 0))
                    PRINTM(MERROR,"del_virt_if: Fail to cancel remain on channel\n");
...
#endif
{
	...
	case NL80211_IFTYPE_P2P_GO:
        ret = woal_cfg80211_add_virt_if(wiphy, name, type, flags, params, &ndev);
		break;
#endif
#endif
    default:
...
#endif
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,2,0) && !defined(COMPAT_WIRELESS)
					params->tail, params->tail_len, NULL, 0, NULL, 0, NULL, 0,
					MGMT_MASK_BEACON
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)
					params->beacon.tail, params->beacon.tail_len,
					params->beacon.proberesp_ies, params->beacon.proberesp_ies_len,
					params->beacon.assocresp_ies, params->beacon.assocresp_ies_len,
#else
					params->tail, params->tail_len,
					params->proberesp_ies, params->proberesp_ies_len,
					params->assocresp_ies, params->assocresp_ies_len,
#endif
					NULL, 0,
					MGMT_MASK_BEACON | MGMT_MASK_PROBE_RESP | MGMT_MASK_ASSOC_RESP
#endif
					)) {
			ret = -EFAULT;
			goto done;
	...
	if (params->beacon.beacon_ies && params->beacon.beacon_ies_len) {
		if (MLAN_STATUS_SUCCESS != woal_cfg80211_mgmt_frame_ie(priv,
				params->beacon.beacon_ies, params->beacon.beacon_ies_len,
		...
		if (params->tail && params->tail_len) {
			if (MLAN_STATUS_SUCCESS !=
					woal_cfg80211_mgmt_frame_ie(priv, params->tail,
						params->tail_len, NULL, 0, NULL, 0, NULL, 0, MGMT_MASK_BEACON)) {
				ret = -EFAULT;
				goto done;
			}
		}
		...
		if (params->proberesp_ies && params->proberesp_ies_len) {
			if (MLAN_STATUS_SUCCESS !=
					woal_cfg80211_mgmt_frame_ie(priv, NULL, 0,
						params->proberesp_ies, params->proberesp_ies_len,
						NULL, 0, NULL, 0, MGMT_MASK_PROBE_RESP)) {
				ret = -EFAULT;
				goto done;
			}
		}
		if (params->assocresp_ies && params->assocresp_ies_len) {
			if (MLAN_STATUS_SUCCESS !=
					woal_cfg80211_mgmt_frame_ie(priv, NULL, 0, NULL, 0,
						params->assocresp_ies, params->assocresp_ies_len, NULL,0,
 ...
 * @return                0 -- success, otherwise fail
 */
int woal_cfg80211_del_beacon(struct wiphy *wiphy,
					struct net_device *dev)
{
	moal_private *priv = (moal_private *) woal_get_netdev_priv(dev);
	int ret = 0;
			...
			u8 *mac, struct station_info *stainfo)
{
    ...
    for(i = 0; i < info->param.sta_list.sta_count; i++){
        if(!memcmp(info->param.sta_list.info[i].mac_address,mac, ETH_ALEN)){
            PRINTM(MIOCTL,"Get station: %02x:%02x:%02x:%02x:%02x:%02x RSSI=%d\n",
					mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],(int)info->param.sta_list.info[i].rssi);
