...
static void woal_sdio_interrupt(struct sdio_func *func)
{
	...
	if (!card || !card->handle) {
		PRINTM(MINFO, "sdio_mmc_interrupt(func = %p) card or handle is NULL, card=%p\n", func, card);
		LEAVE();
		return;
	}
	handle = card->handle;
...
int woal_sdio_suspend(struct device *dev)
{
	...
	if(MLAN_STATUS_SUCCESS == woal_get_pm_info(woal_get_priv(handle, MLAN_BSS_ROLE_ANY),&pm_info)){
...
mlan_status woal_write_reg(moal_handle *handle, t_u32 reg, t_u32 data)
{
	...
	sdio_writeb(((struct sdio_mmc_card *)handle->card)->func, (t_u8)data, reg, (int *)&ret);
	return ret;
}
...
mlan_status woal_read_reg(moal_handle *handle, t_u32 reg, t_u32 *data)
{
	...
	val = sdio_readb(((struct sdio_mmc_card *)handle->card)->func, reg, (int *)&ret);
	*data = val;
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status woal_write_data_sync(moal_handle *handle, mlan_buffer *pmbuf, t_u32 port, t_u32 timeout)
{
	mlan_status ret = MLAN_STATUS_FAILURE;
	...
	t_u32 blkcnt = (blkmode == BLOCK_MODE) ? (pmbuf->data_len/MLAN_SDIO_BLOCK_SIZE) : pmbuf->data_len;
	t_u32 ioport = (port & MLAN_SDIO_IO_PORT_MASK);
#ifdef SDIO_MMC_DEBUG
	handle->cmd53w = 1;
#endif
	if(!sdio_writesb(((struct sdio_mmc_card *)handle->card)->func, ioport, buffer, blkcnt*blksz))
 ...
 *  @return		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status woal_read_data_sync(moal_handle *handle, mlan_buffer *pmbuf, t_u32 port, t_u32 timeout)
{
	mlan_status ret = MLAN_STATUS_FAILURE;
	t_u8 *buffer = (t_u8 *)(pmbuf->pbuf + pmbuf->data_offset);
	t_u8 blkmode = (port & MLAN_SDIO_BYTE_MODE_MASK) ? BYTE_MODE : BLOCK_MODE;
	...
	handle->cmd53r = 1;
#endif
	if(!sdio_readsb(((struct sdio_mmc_card *)handle->card)->func, buffer, ioport, blkcnt*blksz))
		ret = MLAN_STATUS_SUCCESS;
	else
