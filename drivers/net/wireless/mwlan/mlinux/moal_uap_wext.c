                ...
                struct iw_freq *fwrq, char *extra)
{
    ...
    if (MLAN_STATUS_SUCCESS != woal_set_get_sys_config(priv,
                MLAN_ACT_GET, MOAL_IOCTL_WAIT, ap_cfg)) {
    ...
    while (tlv_buf_left >= tlv_hdr_len) {
        if (tlv->element_id == WPA_IE) {
            ...
            if (priv->pairwise_cipher == (CIPHER_TKIP | CIPHER_AES_CCMP)) {
                sys_cfg.wpa_cfg.pairwise_cipher_wpa = CIPHER_TKIP | CIPHER_AES_CCMP;
        ...
        if (tlv->element_id == RSN_IE) {
            ...
            if (priv->pairwise_cipher == (CIPHER_TKIP | CIPHER_AES_CCMP)) {
                sys_cfg.wpa_cfg.pairwise_cipher_wpa2 = (CIPHER_TKIP | CIPHER_AES_CCMP);
            }
            memcpy(priv->bcn_ie_buf + priv->bcn_ie_len,
                    ((t_u8 *)tlv), sizeof(IEEEtypes_Header_t) + tlv->len);
            priv->bcn_ie_len += sizeof(IEEEtypes_Header_t) + tlv->len;
...
#define UAP_RSN_MASK  (BIT(8) | BIT(5) | BIT(1) | BIT(3))
                woal_set_get_custom_ie(priv, UAP_RSN_MASK, priv->bcn_ie_buf, priv->bcn_ie_len)) {
        ...
        if (MLAN_STATUS_SUCCESS !=
                woal_set_get_custom_ie(priv, 0, priv->bcn_ie_buf, priv->bcn_ie_len)) {
                    ...
                    struct iw_point *dwrq, char *extra)
{
    ...
    if ((dwrq->flags & IW_ENCODE_DISABLED) && !ext->key_len) {
        ...
        if(ext->ext_flags & IW_ENCODE_EXT_RX_SEQ_VALID) {
            ...
            DBG_HEXDUMP(MCMD_D,"Uap Rx PN", sec->param.encrypt_key.pn, SEQ_MAX_SIZE);
        }
        if(ext->ext_flags & IW_ENCODE_EXT_TX_SEQ_VALID) {
            memcpy(sec->param.encrypt_key.pn, (t_u8*)ext->tx_seq, SEQ_MAX_SIZE);
            DBG_HEXDUMP(MCMD_D,"Uap Tx PN", sec->param.encrypt_key.pn, SEQ_MAX_SIZE);
        }
        PRINTM(MIOCTL,"set uap wpa key key_index=%d, key_len=%d key_flags=0x%x %02x:%02x:%02x:%02x:%02x:%02x\n",
                           ...
                           char *extra)
{
    ...
    if ((mlme->cmd == IW_MLME_DEAUTH) || (mlme->cmd == IW_MLME_DISASSOC)) {
        ...
        if (!memcmp(bc_addr, sta_addr, ETH_ALEN)) {
            ...
            sta_list = (mlan_ds_sta_list *)kmalloc(
                        sizeof(mlan_ds_sta_list), GFP_KERNEL);
            if (sta_list == NULL) {
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    for(i = 0; i < range->num_frequency; i++) {
        ...
        range->freq[i].m = (long) channel_to_frequency(ap_cfg.
                           chan_list[i].chan_number, band) * 100000;
        range->freq[i].e = 1;
    }
...
struct iw_handler_def woal_uap_handler_def = {
    ...
    num_private_args:sizeof(woal_uap_priv_args) / sizeof(struct iw_priv_args),
