...
woal_cfg80211_set_key(moal_private * priv, t_u8 is_enable_wep,
		t_u32 cipher, const t_u8 *key, int key_len, const t_u8 *seq, int seq_len,
		t_u8 key_index, const t_u8 *addr, int disable)
{
	...
	if (is_enable_wep) {
		...
		if (cipher == WLAN_CIPHER_SUITE_SMS4) {
			...
			if (seq_len) {
				...
				DBG_HEXDUMP(MCMD_D,"WAPI PN", sec->param.encrypt_key.pn, seq_len);
			}
		}
		if (cipher != WLAN_CIPHER_SUITE_WEP40 &&
			cipher != WLAN_CIPHER_SUITE_WEP104) {
						   ...
						   t_u8 index)
{
	...
	if (key_len) {
		if (key_len == 5)
			...
			cipher = WLAN_CIPHER_SUITE_WEP104;
		ret = woal_cfg80211_set_key(priv, 0, cipher, key, key_len, NULL, 0, index, NULL, 0);
	} else {
		/* No key provided so it is enable key. We
		 * want to just set the transmit key index */
		woal_cfg80211_set_key(priv, 1, cipher, key, key_len, NULL, 0, index, NULL, 0);
	}
...
void woal_clear_all_mgmt_ies(moal_private *priv)
{
	...
	if (priv->beacon_wps_index != MLAN_CUSTOM_IE_AUTO_IDX_MASK){
		...
		woal_cfg80211_mgmt_frame_ie(priv, NULL, 0, NULL, 0, NULL, 0, NULL,0,MGMT_MASK_BEACON_WPS_P2P);
	}
	...
	if(mask){
		PRINTM(MCMND,"Clear IES: 0x%x 0x%x 0x%x 0x%x\n",priv->beacon_index,
				priv->probereq_index,priv->proberesp_index,priv->assocresp_index);
		woal_cfg80211_mgmt_frame_ie(priv, NULL, 0, NULL, 0, NULL, 0, NULL,0,mask);
	}
}
...
woal_cfg80211_init_p2p_client(moal_private *priv)
{
	...
	if (MLAN_STATUS_SUCCESS !=
			woal_wifi_direct_mode_cfg(priv,
	...
	if (MLAN_STATUS_SUCCESS !=
		woal_wifi_direct_mode_cfg(priv, MLAN_ACT_SET,
			&wifi_direct_mode)) {
		ret = -EFAULT;
		goto done;
	}
...
woal_cfg80211_deinit_p2p(moal_private *priv)
{
	...
	if(priv->phandle->remain_on_channel){
		...
		if(woal_cfg80211_remain_on_channel_cfg(remain_priv, MOAL_IOCTL_WAIT,MTRUE,&channel_status, NULL, 0, 0)) {
	...
	if (bss_role != MLAN_BSS_ROLE_STA) {
		bss_role = MLAN_BSS_ROLE_STA;
		if (MLAN_STATUS_SUCCESS !=
				woal_cfg80211_bss_role_cfg(priv,
					MLAN_ACT_SET, &bss_role)) {
			ret = -EFAULT;
			goto done;
		}
	}
					...
					struct vif_params *params)
{
	...
	if(priv->phandle->remain_on_channel){
		...
		if(woal_cfg80211_remain_on_channel_cfg(remain_priv,	MOAL_IOCTL_WAIT,MTRUE,&channel_status, NULL, 0, 0)) {
			PRINTM(MERROR,"change_virtual_intf: Fail to cancel remain on channel\n");
			ret = -EFAULT;
			goto done;
		}
		if (priv->phandle->cookie) {
...
woal_cfg80211_set_wiphy_params(struct wiphy *wiphy, u32 changed)
{
	...
	if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP) {
		...
		if ((changed & WIPHY_PARAM_RTS_THRESHOLD) ||
			...
			if (woal_set_get_sys_config(priv,
						MLAN_ACT_SET, MOAL_IOCTL_WAIT, &sys_cfg))
				goto fail;
		}
	}
#endif
	...
	if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_STA) {
		...
		if (changed & WIPHY_PARAM_FRAG_THRESHOLD) {
			if (woal_set_get_frag(priv, MLAN_ACT_SET, MOAL_IOCTL_WAIT, &frag_thr))
				goto fail;
		}
			 ...
			 const t_u8 *mac_addr, struct key_params *params)
{
	...
	if (woal_cfg80211_set_key(priv, 0, params->cipher, params->key,
				 params->key_len, params->seq, params->seq_len, key_index, mac_addr, 0)) {
                                ...
                                )
{
	...
	if (priv) {
		...
		if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_STA) {
			...
			ret =
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0)
                woal_set_rf_channel(priv, chan, channel_type);
#else
                woal_set_rf_channel(priv, chandef->chan, cfg80211_get_chandef_type(chandef));
#endif
		}
#endif
#endif
...
void woal_cfg80211_mgmt_frame_register(struct wiphy *wiphy,
					struct net_device *dev, u16 frame_type, bool reg)
#else
/**
 * @brief register/unregister mgmt frame forwarding
 *
...
#endif
{
	...
	ENTER();
#ifdef UAP_SUPPORT
	if ((priv->bss_type == MLAN_BSS_TYPE_UAP) && (frame_type == IEEE80211_STYPE_PROBE_REQ)) {
					...
					u64 *cookie)
{
	...
	/* frame subtype == probe response, that means we are in listen
	 * phase, so we should not call remain_on_channel_cfg because
	 * remain_on_channl already handled it.
	 *
	 * frame subtype == action, that means we are in PD/GO negotiation,
	 * so we should call remain_on_channel_cfg in order to receive
	 * action frame from peer device */
	framectrl = ((const struct ieee80211_mgmt *) buf)->frame_control;
	PRINTM(MIOCTL,"Mgmt %s: framectrl=0x%x\n", dev->name, framectrl);
	if ((GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP) && (framectrl == IEEE80211_STYPE_PROBE_RESP)){
...
#if defined(WIFI_DIRECT_SUPPORT)
#if LINUX_VERSION_CODE >= WIFI_DIRECT_KERNEL_VERSION
	if ((priv->bss_type == MLAN_BSS_TYPE_WIFIDIRECT) && (framectrl == IEEE80211_STYPE_ACTION)) {
		if (priv->phandle->is_go_timer_set) {
			woal_cancel_timer(&priv->phandle->go_timer);
			priv->phandle->is_go_timer_set = MFALSE;
		}
#define MGMT_TX_DEFAULT_WAIT_TIME	   1000
		...
		if(channel_type_valid)
			ret = woal_cfg80211_remain_on_channel_cfg(priv, MOAL_IOCTL_WAIT, MFALSE,
					&channel_status, chan,channel_type, duration);
		else
#endif
			ret = woal_cfg80211_remain_on_channel_cfg(priv, MOAL_IOCTL_WAIT, MFALSE,
...
#define HEADER_SIZE				8
	...
	pmbuf = woal_alloc_mlan_buffer(priv->phandle,
					MLAN_MIN_DATA_HEADER_LEN +  HEADER_SIZE + packet_len
					+ sizeof(packet_len));
	if (!pmbuf) {
    ...
    *cookie = random32() | 1;
	...
	memcpy(pmbuf->pbuf + pmbuf->data_offset + sizeof(pkt_type), &tx_control, sizeof(tx_control));
...
#else
			cfg80211_mgmt_tx_status(priv->wdev, *cookie, buf, len, true, GFP_ATOMIC);
#endif
#endif
			break;
					...
					custom_ie *probereq_ies_data, t_u16 *probereq_index)
{
	...
	if (!(custom_ie =
				kmalloc(sizeof(mlan_ds_misc_custom_ie), GFP_KERNEL))) {
		ret = -ENOMEM;
		goto done;
	}
	...
	memcpy(&misc->param.cust_ie,
			custom_ie, sizeof(mlan_ds_misc_custom_ie));
 ...
 */
static t_u16 woal_filter_beacon_ies(const t_u8 *ie, int len, t_u8 *ie_out, t_u16 wps_flag)
{
	int left_len = len;
	const t_u8 *pos = ie;
	int length;
		...
		switch(id){
			...
			case VENDOR_SPECIFIC_221:
				...
				if(!memcmp(pVendorIe->vend_hdr.oui, wmm_oui, sizeof(wmm_oui)) )
					break;
				/* filter out wps ie */
				if(!memcmp(pVendorIe->vend_hdr.oui, wps_oui, sizeof(wps_oui)) && (wps_flag & IE_MASK_WPS))
					break;
				/* filter out p2p ie */
				if(!memcmp(pVendorIe->vend_hdr.oui, p2p_oui, sizeof(p2p_oui)) && (wps_flag & IE_MASK_P2P))
					break;
				/* filter out wfd ie */
				if(!memcmp(pVendorIe->vend_hdr.oui, wfd_oui, sizeof(wfd_oui)) && (wps_flag & IE_MASK_WFD))
					break;
		...
		t_u16 mask)
{
	...
	if (beacon_ies_data) {
		...
		if (beacon_ies && beacon_ies_len) {
			...
			beacon_ies_data->ie_length = woal_filter_beacon_ies(beacon_ies,
					beacon_ies_len,beacon_ies_data->ie_buffer,IE_MASK_WPS|IE_MASK_WFD|IE_MASK_P2P);
		} else {
	...
	if (proberesp_ies_data) {
		...
		if (proberesp_ies && proberesp_ies_len) {
			...
			proberesp_ies_data->ie_length = woal_filter_beacon_ies(proberesp_ies,
					proberesp_ies_len,proberesp_ies_data->ie_buffer,IE_MASK_WPS|IE_MASK_WFD);
		} else {
			/* clear the probe response p2p ies */
			if (proberesp_p2p_index > MAX_MGMT_IE_INDEX) {
				PRINTM(MERROR, "Invalid proberesp_p2p_index for mgmt frame ie.\n");
				goto done;
			}
			proberesp_ies_data->ie_index = proberesp_p2p_index;
		...
		if (proberesp_ies && proberesp_ies_len) {
			...
			if(MLAN_CUSTOM_IE_AUTO_IDX_MASK == proberesp_index)
				proberesp_ies_data->mgmt_subtype_mask |= MLAN_CUSTOM_IE_NEW_MASK;
			proberesp_ies_data->ie_length = woal_filter_beacon_ies(proberesp_ies,
					proberesp_ies_len,proberesp_ies_data->ie_buffer,IE_MASK_P2P);
		} else {
	...
	if (probereq_ies_data) {
		...
		if (probereq_ies && probereq_ies_len) {
			...
			if(priv->bss_type != MLAN_BSS_TYPE_WIFIDIRECT){
				...
				probereq_ies_data->ie_length = woal_filter_beacon_ies(probereq_ies,
					probereq_ies_len,probereq_ies_data->ie_buffer,IE_MASK_P2P|IE_MASK_WFD);
			}
			else{
#endif /* KERNEL_VERSION */
#endif /* WIFI_DIRECT_SUPPORT && V14_FEATURE */
