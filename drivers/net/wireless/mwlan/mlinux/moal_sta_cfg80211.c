...
static int woal_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request);
#else
...
static int woal_cfg80211_dump_survey(struct wiphy *wiphy, struct net_device *dev,
...
t_u8 is_cfg80211_special_region_code(char* region_string)
{
    t_u8 i;
    t_u8 size = 0;
    region_code_t cfg80211_special_region_code[] = {{"00 "}, {"99 "},{"98 "},{"97 "}};
 ...
 */
static int
woal_cfg80211_channel_type_to_channel(
		enum nl80211_channel_type channel_type)
{
	int channel;
		...
		switch (element_id) {
			case RSN_IE:
				if (MLAN_STATUS_SUCCESS !=
					woal_set_get_gen_ie(priv, MLAN_ACT_SET, pcurrent_ptr,  &total_ie_len)) {
...
woal_send_domain_info_cmd_fw(moal_private *priv)
{
	...
	if(MTRUE == is_cfg80211_special_region_code(priv->phandle->country_code)){
		PRINTM(MIOCTL,"skip region code config, cfg80211 special region code: %s\n", priv->phandle->country_code);
		goto done;
	}
	/* Allocate an IOCTL request buffer */
				...
				enum nl80211_channel_type channel_type)
{
	...
	if (chan) {
		...
		if (mode == MLAN_BSS_MODE_IBSS){
			...
			radio_cfg->param.band_cfg.adhoc_channel    = ieee80211_frequency_to_channel(chan->center_freq);
		}
		/* Set channel offset */
	...
	PRINTM(MINFO, "Setting band %d, channel bandwidth %d and mode = %d channel=%d\n",
		   config_bands, radio_cfg->param.band_cfg.sec_chan_offset, mode,
		   ieee80211_frequency_to_channel(chan->center_freq));

	if (MLAN_STATUS_SUCCESS !=
			woal_change_adhoc_chan(priv, ieee80211_frequency_to_channel(chan->center_freq))) {
		ret = -EFAULT;
		goto done;
	}
 ...
 */
mlan_status woal_set_ewpa_mode(moal_private * priv, t_u8 wait_option, mlan_ssid_bssid* ssid_bssid)
{
	int             ret = 0;
	mlan_ioctl_req  *req = NULL;
	...
	memcpy(&sec->param.passphrase.ssid, &ssid_bssid->ssid, sizeof(sec->param.passphrase.ssid));
	memcpy(&sec->param.passphrase.bssid, &ssid_bssid->bssid,MLAN_MAC_ADDR_LENGTH);
		...
		for (i = 0; i < scan_resp.num_in_scan_table; i++) {
			if (ssid_bssid) {
				...
				if (memcmp(ssid_bssid->ssid.ssid, scan_table[i].ssid.ssid,
					 ...
					 memcmp(ssid_bssid->bssid,scan_table[i].mac_address,ETH_ALEN))
					continue;
			}
			if (!scan_table[i].freq) {
				scan_table[i].freq = ieee80211_channel_to_frequency((int)scan_table[i].channel,
						woal_band_cfg_to_ieee_band(scan_table[i].bss_band));
			}
			chan = ieee80211_get_channel(priv->wdev->wiphy, scan_table[i].freq);
			if(!chan){
				PRINTM(MERROR, "Fail to get chan with freq: channel=%d freq=%d\n",
			...
			pub = cfg80211_inform_bss(priv->wdev->wiphy, chan,
						...
						scan_table[i].pbeacon_buf + WLAN_802_11_FIXED_IE_SIZE,
						scan_table[i].beacon_buf_size - WLAN_802_11_FIXED_IE_SIZE,
						-RSSI_DBM_TO_MDM(scan_table[i].rssi), GFP_KERNEL);
			if(pub){
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0)
							  ...
							  t_u16 beacon_interval)
{
	...
	bss = cfg80211_inform_bss(priv->wdev->wiphy, chan,
		bss_info.bssid, 0, WLAN_CAPABILITY_IBSS, beacon_interval,
		ie_buf, ie_len, signal.bcn_rssi_avg, GFP_KERNEL);
	if(bss)
...
woal_cfg80211_assoc(moal_private *priv, void *sme)
{
	...
	if ((mode == MLAN_BSS_MODE_IBSS) && channel) {
		...
		if (MLAN_STATUS_SUCCESS != woal_set_rf_channel(priv,
				channel, woal_channel_to_nl80211_channel_type(
				radio_cfg->param.band_cfg.sec_chan_offset))) {
			ret = -EFAULT;
			goto done;
		}
	...
	if (conn_param && mode != MLAN_BSS_MODE_IBSS) {
		...
		if (conn_param->crypto.n_ciphers_pairwise) {
			pairwise_enc_mode =
				woal_cfg80211_get_encryption_mode(conn_param->crypto.
			...
			if (alg_is_wep) {
				...
				if (MLAN_STATUS_SUCCESS !=
					woal_cfg80211_set_wep_keys(priv, conn_param->key,
							conn_param->key_len, conn_param->key_idx)) {
					ret = -EFAULT;
					goto done;
				}
 ...
 *  @return                     MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING -- success, otherwise fail
 */
static mlan_status woal_set_get_dtim_period(moal_private * priv,
 ...
 */
static mlan_status
woal_cfg80211_dump_station_info(moal_private *priv,
			  struct station_info *sinfo)
{
	...
	t_u16 Rates[12] = {0x02, 0x04, 0x0B, 0x16,0x0C, 0x12,0x18, 0x24, 0x30, 0x48, 0x60,0x6c};
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0) || defined(COMPAT_WIRELESS)
	mlan_bss_info bss_info;
	t_u8 dtim_period = 0;
#endif
	...
	if (rate->param.data_rate.tx_data_rate >= MLAN_RATE_INDEX_MCS0) {
		...
		sinfo->txrate.mcs = rate->param.data_rate.tx_data_rate - MLAN_RATE_INDEX_MCS0;
	}
	else {
...
#if defined(WIFI_DIRECT_SUPPORT)
#if LINUX_VERSION_CODE >= WIFI_DIRECT_KERNEL_VERSION
    if (priv->bss_type == MLAN_BSS_TYPE_WIFIDIRECT && priv->scan_request->n_ssids) {
	...
	if (priv->scan_request->ie && priv->scan_request->ie_len) {
		if (MLAN_STATUS_SUCCESS !=
			woal_cfg80211_mgmt_frame_ie(priv, NULL, 0,
				NULL,0, NULL, 0,
				(t_u8*)priv->scan_request->ie, priv->scan_request->ie_len,
				MGMT_MASK_PROBE_REQ)) {
		...
		if (priv->probereq_index != MLAN_CUSTOM_IE_AUTO_IDX_MASK)
			woal_cfg80211_mgmt_frame_ie(priv, NULL, 0, NULL, 0, NULL, 0, NULL, 0, MGMT_MASK_PROBE_REQ);
	}
			 ...
			 struct cfg80211_connect_params *sme)
{
	...
	if (!ret) {
		...
		PRINTM(MINFO, "Association to bssid %pM failed\n",
			priv->cfg_bssid);
		cfg80211_connect_result(priv->netdev, priv->cfg_bssid, NULL, 0,
			NULL, 0, woal_get_assoc_status(priv), GFP_KERNEL);
	...
	int idx, struct survey_info *survey)
{
	...
	survey->channel = ieee80211_get_channel(wiphy,ieee80211_channel_to_frequency(bss_info.bss_chan,band));
					...
					t_u32 duration)
{
	...
	if(MLAN_STATUS_SUCCESS == woal_set_remain_channel_ioctl(priv, wait_option, &chan_cfg))
...
#endif
{
	...
	if (priv->phandle->remain_on_channel) {
		...
		if (woal_cfg80211_remain_on_channel_cfg(remain_priv,MOAL_IOCTL_WAIT, MTRUE, &status, NULL, 0, 0)) {
			PRINTM(MERROR, "mgmt_tx_cancel_wait: Fail to cancel remain on channel\n");
			ret = -EFAULT;
			goto done;
		}
...
#endif
{
	...
	if(priv->phandle->remain_on_channel) {
		...
		if(woal_cfg80211_remain_on_channel_cfg(remain_priv,	MOAL_IOCTL_WAIT,MTRUE,&status, NULL, 0, 0)) {
			PRINTM(MERROR,"remain_on_channel: Fail to cancel remain on channel\n");
			ret = -EFAULT;
			goto done;
		}
		priv->phandle->cookie = 0;
...
#endif
{
	...
	if(woal_cfg80211_remain_on_channel_cfg(remain_priv, MOAL_IOCTL_WAIT,MTRUE,&status, NULL, 0, 0)) {
		PRINTM(MERROR,"cancel_remain_on_channel: Fail to cancel remain on channel\n");
		ret = -EFAULT;
		goto done;
	}
                                ...
                                struct cfg80211_sched_scan_request *request)
{
...
#endif
	PRINTM(MIOCTL,"%s sched scan: n_ssids=%d n_match_sets=%d n_channels=%d interval=%d ie_len=%d\n",
	...
	if(MLAN_STATUS_SUCCESS == woal_request_bgscan(priv, MOAL_IOCTL_WAIT, &priv->scan_cfg)){
...
int  woal_cfg80211_resume(struct wiphy *wiphy)
{
    ...
    for (i = 0; i < MIN(handle->priv_num, MLAN_MAX_BSS_NUM); i++) {
        if (handle->priv[i] &&
            ...
            if ((handle->priv[i]->last_event & EVENT_BG_SCAN_REPORT) && handle->priv[i]->sched_scanning){
                woal_inform_bss_from_scan_result(handle->priv[i], NULL,MOAL_CMD_WAIT);
 ...
 * @param sme             A pointer to cfg80211_connect_params structure
 */
void woal_save_conn_params(moal_private *priv,struct cfg80211_connect_params *sme)
{
    ENTER();
    memcpy(&priv->sme_current, sme, sizeof(struct cfg80211_connect_params));
    if (sme->channel) {
        priv->sme_current.channel = &priv->conn_chan;
        memcpy(priv->sme_current.channel, sme->channel, sizeof(struct ieee80211_channel));
    }
    if (sme->bssid) {
...
void woal_start_roaming(moal_private *priv)
{
    ...
    memcpy(ssid_bssid.ssid.ssid,priv->sme_current.ssid, priv->sme_current.ssid_len);
    ...
    PRINTM(MIOCTL,"Find AP: bssid=%pM, signal=%d\n", &ssid_bssid.bssid, ssid_bssid.rssi);
    /* check signal */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0) || defined(COMPAT_WIRELESS)
        cfg80211_roamed(priv->netdev, NULL, priv->cfg_bssid, priv->sme_current.ie, priv->sme_current.ie_len,
                assoc_rsp.assoc_resp_buf, assoc_rsp.assoc_resp_len, GFP_KERNEL);
#else
        cfg80211_roamed(priv->netdev, priv->cfg_bssid, priv->sme_current.ie, priv->sme_current.ie_len,
                assoc_rsp.assoc_resp_buf, assoc_rsp.assoc_resp_len, GFP_KERNEL);
#endif
		PRINTM(MIOCTL, "Roamed to bssid %pM successfully\n",priv->cfg_bssid);
    }
    else{
        PRINTM(MIOCTL, "Roaming to bssid %pM failed\n",&ssid_bssid.bssid);
    }
done:
...
#define SPECIFIC_SCAN_CHAN_TIME      40
	woal_set_scan_time(priv, ACTIVE_SCAN_CHAN_TIME, PASSIVE_SCAN_CHAN_TIME, SPECIFIC_SCAN_CHAN_TIME);
	...
	if (MLAN_STATUS_SUCCESS == woal_set_get_power_mgmt(priv,MLAN_ACT_GET, &disabled, 0)) {
...
mlan_status woal_cfg80211_init_wiphy(moal_private *priv, t_u8 wait_option)
{
	...
	if (frag_thr < MLAN_RTS_MIN_VALUE ||
		frag_thr > MLAN_RTS_MAX_VALUE)
...
mlan_status woal_register_cfg80211(moal_private *priv)
{
	...
	wiphy->interface_modes = MBIT(NL80211_IFTYPE_STATION) |	MBIT(NL80211_IFTYPE_ADHOC) |
	...
	if (MLAN_STATUS_SUCCESS == woal_request_get_fw_info(priv, MOAL_CMD_WAIT, &fw_info)) {
