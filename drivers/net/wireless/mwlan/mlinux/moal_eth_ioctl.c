...
static t_u8  SupportedInfraBand[] = {
    ...
    BAND_A|BAND_B|BAND_G|BAND_AN|BAND_GN,  BAND_A|BAND_G|BAND_AN|BAND_GN, BAND_A|BAND_AN,
};
 ...
 */
mlan_status parse_arguments(t_u8 *pos, int* data, int datalen, int *user_data_len)
{
    unsigned int i, j, k;
    char cdata[10];
    int is_hex = 0;
 ...
 */
mlan_status woal_set_ap_wps_p2p_ie(moal_private * priv, t_u8 *ie,
        size_t len)
{
    ...
    */
    if (*pos == '1') {
        ...
        pos += 2;
        if (MLAN_STATUS_SUCCESS != woal_cfg80211_mgmt_frame_ie(priv, pos, ie_len,
					NULL, 0, NULL, 0, NULL, 0, MGMT_MASK_BEACON_WPS_P2P)) {
        ...
        if (MLAN_STATUS_SUCCESS != woal_cfg80211_mgmt_frame_ie(priv, NULL,
                    0, pos, ie_len, NULL, 0, NULL, 0,  MGMT_MASK_PROBE_RESP)) {
        ...
        pos += 2;
        if (MLAN_STATUS_SUCCESS != woal_cfg80211_mgmt_frame_ie(priv, NULL,
                    0, NULL, 0, pos, ie_len, NULL, 0,  MGMT_MASK_ASSOC_RESP)) {
 ...
 */
int woal_get_priv_driver_version(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int             len = 0, ret = -1;
...
int woal_priv_hostcmd(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    ret = misc_cfg->param.hostcmd.len + sizeof(buf_len) + strlen(CMD_MARVELL) + strlen(PRIV_CMD_HOSTCMD);
...
int woal_priv_customie(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if ((custom_ie->len == 0) || (custom_ie->len == sizeof(custom_ie->ie_data_list[0].ie_index)))
    ...
    if (MLAN_STATUS_SUCCESS != woal_request_ioctl(priv, ioctl_req, MOAL_IOCTL_WAIT)) {
        ret =  -EFAULT;
        goto done;
    }
    custom_ie = (mlan_ds_misc_custom_ie *)data_ptr;
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_bandcfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    unsigned int i;
    int data[4];
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_BANDCFG))) {
        ...
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_BANDCFG), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (user_data_len == 0) {
        ...
        if(user_data_len == 4) {
            if(!(adhoc_band & (BAND_GN
                ...
                PRINTM(MERROR,"11n is not enabled for adhoc, can not set HT/VHT channel bandwidth\n");
                ret = -EINVAL;
                goto error;
            }
            ...
            if((adhoc_chan_bandwidth != CHANNEL_BW_20MHZ) &&
               ...
               (adhoc_chan_bandwidth != CHANNEL_BW_40MHZ_BELOW)
                ...
                PRINTM(MERROR,"Invalid secondary channel bandwidth, only allowed 0, 1, 3 or 4\n");
                ret = -EINVAL;
                goto error;
            }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_httxcfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[2];
    mlan_ioctl_req  *req = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HTTXCFG))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_HTTXCFG), data, sizeof(data) / sizeof(int), &user_data_len);
    }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_htcapinfo(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int data[2];
    mlan_ioctl_req  *req = NULL;
    mlan_ds_11n_cfg *cfg_11n = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HTCAPINFO))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_HTCAPINFO), data, sizeof(data) / sizeof(int), &user_data_len);
    }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_addbapara(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int data[5];
    mlan_ioctl_req  *req = NULL;
    mlan_ds_11n_cfg *cfg_11n = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_ADDBAPARA))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_ADDBAPARA), data, sizeof(data) / sizeof(int), &user_data_len);
        ...
        if(data[1] <= 0 || data[1] > MLAN_AMPDU_MAX_TXWINSIZE || data[2] <= 0 || data[2] > MLAN_AMPDU_MAX_RXWINSIZE) {
    ...
    if (user_data_len == 0) {
        ...
        PRINTM(MINFO, "SET: timeout:%d txwinsize:%d rxwinsize:%d txamsdu=%d rxamsdu=%d\n",
    ...
    PRINTM(MINFO, "GET: timeout:%d txwinsize:%d rxwinsize:%d txamsdu=%d, rxamsdu=%d\n",
...
#define MAC_STRING_LENGTH   17
    ...
    parse_arguments(respbuf + header_len, data, sizeof(data) / sizeof(t_u32), &user_data_len);
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_rejectaddbareq(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[1];
    mlan_ioctl_req  *req = NULL;
    mlan_ds_11n_cfg *cfg_11n = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_REJECTADDBAREQ))) {
        /* GET operation */
        user_data_len = 0;
    } else {
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_REJECTADDBAREQ), data, sizeof(data) / sizeof(int), &user_data_len);
    }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_aggrpriotbl(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_AGGRPRIOTBL))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_AGGRPRIOTBL), data, sizeof(data) / sizeof(int), &user_data_len);
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_addbareject(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_ADDBAREJECT))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_ADDBAREJECT), data, sizeof(data) / sizeof(int), &user_data_len);
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_txratecfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[3];
    mlan_ioctl_req   *req = NULL;
    mlan_ds_rate     *rate = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_TXRATECFG))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_TXRATECFG), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (rate->param.rate_cfg.is_rate_auto == MTRUE) {
        ...
        if (rate->param.rate_cfg.rate < MLAN_RATE_INDEX_MCS0) {
            ...
            ratecfg->rate_index = rate->param.rate_cfg.rate - MLAN_RATE_INDEX_MCS0;
        }
    }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_esuppmode(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[3];
    mlan_ioctl_req   *req = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_ESUPPMODE))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_ESUPPMODE), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    esupp_mode->pairwise_cipher = (t_u8) ((sec->param.esupp_mode.act_paircipher) & 0xFF);
    esupp_mode->group_cipher = (t_u8) ((sec->param.esupp_mode.act_groupcipher) & 0xFF);
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_setget_priv_passphrase(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    mlan_ioctl_req   *req = NULL;
    mlan_ds_sec_cfg  *sec = NULL;
    ...
    while(begin) {
        ...
        else if (!strnicmp(opt, "psk", strlen(opt)) && req->action == MLAN_ACT_SET) {
            if (strlen(end) != MLAN_PMK_HEXSTR_LENGTH) {
                PRINTM(MERROR, "Invalid PMK length\n");
                ret = -EINVAL;
                break;
            }
            woal_ascii2hex((t_u8 *)(sec->param.passphrase.psk.pmk.pmk), end, MLAN_PMK_HEXSTR_LENGTH / 2);
            sec->param.passphrase.psk_type = MLAN_PSK_PMK;
        }
        else if (!strnicmp(opt, "passphrase", strlen(opt)) && req->action == MLAN_ACT_SET) {
            if (strlen(end) < MLAN_MIN_PASSPHRASE_LENGTH  || strlen(end) > MLAN_MAX_PASSPHRASE_LENGTH ) {
                PRINTM(MERROR,"Invalid length for passphrase\n");
                ret = -EINVAL;
                break;
            }
            sec->param.passphrase.psk_type = MLAN_PSK_PASSPHRASE;
            strncpy(sec->param.passphrase.psk.passphrase.passphrase, end, sizeof(sec->param.passphrase.psk.passphrase.passphrase));
    ...
    if(sec->param.passphrase.ssid.ssid_len) {
        ...
        memcpy(respbuf +len, sec->param.passphrase.ssid.ssid,sec->param.passphrase.ssid.ssid_len);
    ...
    if(sec->param.passphrase.psk_type == MLAN_PSK_PMK) {
        ...
        for (i=0; i<MLAN_MAX_KEY_LENGTH; ++i)
            len += sprintf(respbuf+len,"%02x",sec->param.passphrase.psk.pmk.pmk[i]);
        len += sprintf(respbuf+len, "\n");
    }
...
int woal_priv_deauth(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) > (strlen(CMD_MARVELL) + strlen(PRIV_CMD_DEAUTH))) {
        ...
        woal_mac2u8(mac, respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_DEAUTH));
 ...
 *  @return             Number of bytes written, negative for failure.
 */
static int woal_priv_ap_deauth(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u8               *data_ptr;
    mlan_ioctl_req  *ioctl_req = NULL;
    ...
    PRINTM(MIOCTL,"ioctl deauth station: %02x:%02x:%02x:%02x:%02x:%02x, reason=%d\n",deauth_param.mac_addr[0],
        deauth_param.mac_addr[1],deauth_param.mac_addr[2],deauth_param.mac_addr[3],deauth_param.mac_addr[4],
        deauth_param.mac_addr[5], deauth_param.reason_code);
 ...
 */
static int woal_priv_get_sta_list(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    ...
    ioctl_req = (mlan_ioctl_req *) woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_get_info));
    if (ioctl_req == NULL){
        ret = -ENOMEM;
        goto done;
    }
    ...
    sta_list = (mlan_ds_sta_list *)(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_GET_STA_LIST));
 ...
 *  @return         0 --success, otherwise fail
 */
static int woal_priv_bss_config(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    mlan_ds_bss *bss = NULL;
    mlan_ioctl_req  *ioctl_req = NULL;
    ...
    /* Allocate an IOCTL request buffer */
    ioctl_req = (mlan_ioctl_req *) woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_bss));
    if (ioctl_req == NULL){
        ret = -ENOMEM;
        goto done;
    }
    ...
    if (action == 1) {
        ...
        memcpy(&bss->param.bss_config, respbuf + offset, sizeof(mlan_uap_bss_param));
    } else {
        ioctl_req->action = MLAN_ACT_GET;
    }
    ...
    if (ioctl_req->action == MLAN_ACT_GET) {
        memcpy(respbuf + offset, &bss->param.bss_config, sizeof(mlan_uap_bss_param));
    }
...
int woal_priv_bssrole(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_BSSROLE))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_BSSROLE), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (MLAN_STATUS_SUCCESS != woal_bss_role_cfg(priv,
            action, MOAL_IOCTL_WAIT, (t_u8 *)data)) {
        ret = -EFAULT;
        goto error;
    }
...
int woal_priv_setuserscan(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    memcpy((char *)&scan_cfg, respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_SETUSERSCAN), sizeof(wlan_user_scan_cfg));
 ...
 *  @return             MLAN_STATUS_SUCCESS --success, otherwise fail
 */
int moal_ret_get_scan_table_ioctl(t_u8 *respbuf, t_u32 respbuflen, mlan_scan_resp *scan_resp,
                                         t_u32 scan_start)
{
...
int woal_priv_getscantable(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    memcpy(&scan_start, respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_GETSCANTABLE), sizeof(scan_start));
    if (scan_start) {
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_setgetdeepsleep(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[2];
    int ret = 0;
    int user_data_len = 0;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_DEEPSLEEP))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_DEEPSLEEP), data, sizeof(data) / sizeof(int), &user_data_len);
    }
...
int woal_priv_setgetipaddr(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (data_length < 1) { /* GET */
        ...
        in4_pton(&respbuf[header + 2], MIN((IPADDR_MAX_BUF - 3), (data_length - 2)),
...
int woal_priv_setwpssession(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_WPSSESSION))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_WPSSESSION), data, sizeof(data) / sizeof(int), &user_data_len);
    }
...
int woal_priv_otpuserdata(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    memset ((char *)data, 0, sizeof(data));
    parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_OTPUSERDATA), data, sizeof(data) / sizeof(int), &user_data_len);
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_set_get_countrycode(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int  ret = 0;
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_setgettcpackenh(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    t_u32 data[1];
    int ret = 0;
    int user_data_len = 0;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_TCPACKENH))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_TCPACKENH), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (user_data_len == 0) {
        ...
        if (data[0] == MTRUE) {
            ...
            while (!list_empty(&priv->tcp_sess_queue))
            {
                ...
                PRINTM(MINFO, "Disable TCP ACK Enh: release a tcp session in dl. (src: ipaddr 0x%08x, port: %d. dst: ipaddr 0x%08x, port: %d\n", tcp_sess->src_ip_addr, tcp_sess->src_tcp_port, tcp_sess->dst_ip_addr, tcp_sess->dst_tcp_port);
                list_del(link);
                kfree(tcp_sess);
            }
        } else {
...
int woal_priv_assocessid(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_ASSOCESSID))) {
        ...
        /* SET operation */
       essid_ptr = respbuf + (strlen(CMD_MARVELL) + strlen(PRIV_CMD_ASSOCESSID));
    ...
    /* Set the SSID */
    memcpy(req_ssid.ssid, essid_ptr, MIN(req_ssid.ssid_len, MLAN_MAX_SSID_LENGTH));
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_getwakeupreason(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    mlan_ioctl_req   *req = NULL;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_WAKEUPREASON))) {
        /* GET operation */
        req = woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_misc_cfg));
        if(req == NULL) {
            LEAVE();
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_set_get_listeninterval(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int data[1];
    int user_data_len = 0;
    int  ret = 0;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_LISTENINTERVAL))) {
        /* GET operation */
        user_data_len = 0;
    } else {
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_LISTENINTERVAL), data, sizeof(data) / sizeof(int), &user_data_len);
    }
 ...
 *  @return             Number of bytes written, negative for failure.
 */
int woal_priv_set_get_drvdbg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int data[4];
    int user_data_len = 0;
    int ret = 0;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_DRVDBG))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_DRVDBG), data, sizeof(data) / sizeof(int), &user_data_len);
    }
 ...
 *  @return             0 --success, otherwise fail
 */
int woal_priv_hscfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen, BOOLEAN invoke_hostcmd)
{
    int data[3];
    int user_data_len = 0;
    int ret = 0;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HSCFG))) {
        ...
        parse_arguments(arguments, data, sizeof(data) / sizeof(int), &user_data_len);
    ...
    if (user_data_len && (data[0] != HOST_SLEEP_CFG_CANCEL || invoke_hostcmd == MFALSE)) {
    ...
    if ((invoke_hostcmd == MTRUE) && (action == MLAN_ACT_SET)) {
        ...
        if (MLAN_STATUS_SUCCESS != woal_set_get_hs_params(priv, MLAN_ACT_SET, MOAL_IOCTL_WAIT, &hscfg)) {
            ret = -EFAULT;
            goto done;
        }
    }
    hscfg.is_invoke_hostcmd = invoke_hostcmd;
    if (MLAN_STATUS_SUCCESS != woal_set_get_hs_params(priv, action, MOAL_IOCTL_WAIT, &hscfg)) {
        ret = -EFAULT;
        goto done;
    }
...
int woal_priv_hssetpara(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HSSETPARA))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_HSSETPARA), data, sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (user_data_len >= 1 && user_data_len <= 3) {
        sprintf(respbuf, "%s%s%s", CMD_MARVELL, PRIV_CMD_HSCFG, respbuf+(strlen(CMD_MARVELL)+strlen(PRIV_CMD_HSSETPARA)));
 ...
 * @return         0 --success, otherwise fail
 */
int woal_priv_set_get_scancfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    int user_data_len = 0;
    int arg_len = 7;
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_SCANCFG))) {
        ...
        /* SET operation */
        memset((char *)data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_SCANCFG), data, sizeof(data) / sizeof(int), &user_data_len);
    }
...
static int woal_priv_set_ap(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    memcpy(&priv->prev_ssid_bssid.ssid, &bss_info.ssid, sizeof(mlan_802_11_ssid));
 ...
 * @return         Number of bytes written if successful, otherwise fail
 */
static int woal_priv_set_bss_mode(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
        int             ret = 0;
        mlan_ds_bss     *bss = NULL;
        mlan_ioctl_req  *req = NULL;
 ...
 * @return         Number of bytes written if successful, otherwise fail
 */
static int woal_priv_set_power(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    struct mwreq *mwr;
    t_u8            *data_ptr;
    int             ret = 0, disabled;
    ...
    if (MLAN_STATUS_SUCCESS != woal_set_get_power_mgmt(priv,
        MLAN_ACT_SET, &disabled, mwr->u.power.flags)) {
        return -EFAULT;
    }
    LEAVE();
    return ret;
 ...
 * @return         Number of bytes written if successful, otherwise fail
 */
static int woal_priv_set_essid(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    mlan_802_11_ssid   req_ssid;
    mlan_ssid_bssid    ssid_bssid;
#ifdef REASSOCIATION
    ...
    if (!req_ssid.ssid_len) {
        ...
        /* Set the SSID */
        memcpy(req_ssid.ssid, mwr->u.essid.pointer, MIN(req_ssid.ssid_len, MLAN_MAX_SSID_LENGTH));
        if (!req_ssid.ssid_len || (MFALSE == woal_ssid_valid(&req_ssid))) {
            PRINTM(MERROR, "Invalid SSID - aborting set_essid\n");
            ret = -EINVAL;
            goto setessid_ret;
    ...
    if (mode != MW_MODE_ADHOC) {
        if (MLAN_STATUS_SUCCESS != woal_find_best_network(priv,MOAL_IOCTL_WAIT,&ssid_bssid)) {
            ret = -EFAULT;
            goto setessid_ret;
        }
    }
    else if (MLAN_STATUS_SUCCESS != woal_find_best_network(priv,MOAL_IOCTL_WAIT,&ssid_bssid))
 ...
 * @return         Number of bytes written if successful, otherwise fail
 */
static int woal_priv_set_auth(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    struct mwreq *mwr;
    t_u8            *data_ptr;
    int             ret = 0;
    ...
    switch (mwr->u.param.flags & MW_AUTH_INDEX) {
        ...
        case MW_AUTH_CIPHER_GROUP:
            ...
            if(MLAN_STATUS_SUCCESS != woal_set_encrypt_mode(priv,MOAL_IOCTL_WAIT,encrypt_mode))
                ret =  -EFAULT;
            break;
        case MW_AUTH_80211_AUTH_ALG:
            ...
            if(MLAN_STATUS_SUCCESS != woal_set_auth_mode(priv,MOAL_IOCTL_WAIT,auth_mode))
                ret =  -EFAULT;
            break;
        case MW_AUTH_WPA_ENABLED:
            if(MLAN_STATUS_SUCCESS != woal_set_wpa_enable(priv,MOAL_IOCTL_WAIT,mwr->u.param.value))
                ret =  -EFAULT;
            break;
#define MW_AUTH_WAPI_ENABLED    0x20
        case MW_AUTH_WAPI_ENABLED:
            if(MLAN_STATUS_SUCCESS != woal_set_wapi_enable(priv,MOAL_IOCTL_WAIT,mwr->u.param.value))
                ret =  -EFAULT;
            break;
...
static int woal_priv_get_ap(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if ((ret = woal_get_bss_info(priv, MOAL_IOCTL_WAIT, &bss_info)) != MLAN_STATUS_SUCCESS) {
        return -EFAULT;
    }
 ...
 * @return         Number of bytes written if successful else negative value
 */
static int woal_priv_get_power(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    struct mwreq *mwr;
    t_u8            *data_ptr;
    int             ret = 0, ps_mode;
    ...
    mwr->u.power.value = 0;
    ret = strlen(CMD_MARVELL) + strlen(PRIV_CMD_GET_POWER) + sizeof(struct mwreq);
    LEAVE();
    return ret;
}
#endif /* STA_SUPPORT */
 ...
 * @return         Number of bytes written if successful else negative value
 */
static int woal_priv_warmreset(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int             ret = 0;
    int intf_num;
    moal_handle *handle = priv->phandle;
    ...
    for (intf_num = 0; intf_num < handle->priv_num; intf_num++) {
        ...
        if ( (handle->priv[intf_num]->bss_type == MLAN_BSS_TYPE_WIFIDIRECT) &&
            ...
            if (MLAN_STATUS_SUCCESS != woal_bss_role_cfg(handle->priv[intf_num],
                    MLAN_ACT_SET, MOAL_IOCTL_WAIT, &bss_role)) {
			ret = -EFAULT;
			goto done;
	        }
        }
 ...
 *  @return         0 --success, otherwise fail
 */
static int woal_priv_txpowercfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int data[4];
	int user_data_len;
	int ret = 0;
		...
		switch (user_data_len) {
			...
			case 4:
				...
				if (data[1] < bss_info.min_power_level) {
					PRINTM(MERROR, "The set powercfg rate value %d dBm is out of range (%d dBm-%d dBm)!\n",
						data[1], (int)bss_info.min_power_level,(int)bss_info.max_power_level);
					ret = -EINVAL;
					break;
				}
				if (user_data_len == 4) {
					...
					if (data[2] > bss_info.max_power_level) {
						PRINTM(MERROR, "The set powercfg rate value %d dBm is out of range (%d dBm-%d dBm)!\n",
							data[2], (int)bss_info.min_power_level,(int)bss_info.max_power_level);
						ret = -EINVAL;
						break;
					}
					if (data[3] > data[2] - data[1]) {
						PRINTM(MERROR, "Step should not greater than power difference!\n");
						ret = -EINVAL;
						break;
					}
				}
	...
	if(!user_data_len) {
		/* GET operation */
		memcpy(respbuf, (t_u8*)&pcfg->param.power_ext, sizeof(pcfg->param.power_ext));
...
static int woal_priv_pscfg(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (user_data_len) {
		...
		if ((data[i] != SPECIAL_ADHOC_AWAKE_PD) && ((data[i] < MIN_ADHOC_AWAKE_PD) || (data[i] > MAX_ADHOC_AWAKE_PD))) {
			PRINTM(MERROR, "Invalid argument for adhoc awake period\n");
			ret = -EINVAL;
			goto done;
		}
		i++;
		if ((data[i] != DISABLE_BCN_MISS_TO) && ((data[i] < MIN_BCN_MISS_TO) || (data[i] > MAX_BCN_MISS_TO))) {
		...
		i++;
		if ((data[i] != PS_MODE_UNCHANGED) && (data[i] != PS_MODE_AUTO) && (data[i] != PS_MODE_POLL) && (data[i] != PS_MODE_NULL)) {
		...
		i++;
		req->action = MLAN_ACT_SET;
		memcpy(&pm_cfg->param.ps_cfg,data,MIN(sizeof(pm_cfg->param.ps_cfg), sizeof(data)));
	}
	else
		req->action = MLAN_ACT_GET;
	...
	memcpy(data, &pm_cfg->param.ps_cfg, MIN((sizeof(int) * allowed), sizeof(pm_cfg->param.ps_cfg)));
 ...
 * @return         0 --success, otherwise fail
 */
static int woal_priv_sleeppd(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int		ret = 0;
	mlan_ds_pm_cfg *pm_cfg = NULL;
	mlan_ioctl_req	  *req = NULL;
    ...
    if (strlen(respbuf) == header_len) {
        ...
        /* SET operation */
        parse_arguments(respbuf + header_len, &data, sizeof(data) / sizeof(int), &user_data_len);
    }
	...
	if (user_data_len) {
		if ((data <= MAX_SLEEP_PERIOD && data >= MIN_SLEEP_PERIOD) || (data == 0)
 ...
 * @return         0 --success, otherwise fail
 */
static int woal_priv_txcontrol(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    mlan_ds_misc_cfg *misc_cfg = NULL;
    mlan_ioctl_req *req = NULL;
 ...
 * @return         0 --success, otherwise fail
 */
static int woal_priv_regrdwr(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int data[3];
	int ret = 0;
	mlan_ioctl_req *req = NULL;
 ...
 * @return         0 --success, otherwise fail
 */
static int woal_priv_rdeeprom(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int data[2];
	int ret = 0;
	mlan_ioctl_req *req = NULL;
	...
	if (req->action == MLAN_ACT_GET) {
		memcpy(respbuf, &reg_mem->param.rd_eeprom, sizeof(reg_mem->param.rd_eeprom));
 ...
 * @return         0 --success, otherwise fail
 */
static int woal_priv_memrdwr(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int data[2];
	int ret = 0;
	mlan_ioctl_req *req = NULL;
 ...
 *  @return             MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static int woal_priv_sdcmd52rw(moal_private* priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    parse_arguments(respbuf + header_len, buf, sizeof(buf) / sizeof(int), &user_data_len);
    ...
    if(!rw) {
        ...
        if (func)
            data = sdio_readb(((struct sdio_mmc_card *)priv->phandle->card)->func, reg, &ret);
        else
            data = sdio_f0_readb(((struct sdio_mmc_card *)priv->phandle->card)->func, reg, &ret);
        ...
        sdio_claim_host(((struct sdio_mmc_card *)priv->phandle->card)->func);
        if (func)
            sdio_writeb(((struct sdio_mmc_card *)priv->phandle->card)->func, data, reg, &ret);
        else
            sdio_f0_writeb(((struct sdio_mmc_card *)priv->phandle->card)->func, data, reg, &ret);
        sdio_release_host(((struct sdio_mmc_card *)priv->phandle->card)->func);
        if (ret) {
 ...
 *  @return		0 --success, otherwise fail
 */
static int woal_priv_arpfilter(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int	             ret = 0;
	mlan_ds_misc_cfg *misc = NULL;
	mlan_ioctl_req   *req = NULL;
    ...
    memcpy((void*)(misc->param.gen_ie.ie_data), data_ptr+sizeof(buf_len), buf_len);
 ...
 *  @return         0 --success, otherwise fail
 */
int woal_priv_mgmt_frame_passthru_ctrl(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    int ret = 0;
    int data = 0;
	int user_data_len = 0, header_len = 0;
 ...
 *  @return         0 --success, otherwise fail
 */
static int woal_priv_qconfig(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    mlan_ioctl_req  *req = NULL;
...
int woal_priv_offchannel(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
    ...
    if (user_data_len == 2) {
        if (data[0] == 1) {
            ...
            PRINTM(MWARN, "extra arguments are ignored since action is 'cancel'\n");
        }
    }
    if (user_data_len == 3) {
        if (data[0] == 1) {
    ...
    if (data[0] == 0) {
        if (woal_cfg80211_remain_on_channel_cfg(priv, MOAL_IOCTL_WAIT, MTRUE, &status, NULL, 0, 0)) {
			PRINTM(MERROR,"remain_on_channel: Failed to cancel\n");
			ret = -EFAULT;
            goto done;
        }
    } else if (data[0] == 1) {
        if (woal_cfg80211_remain_on_channel_cfg(priv, MOAL_IOCTL_WAIT, MFALSE, &status, ieee80211_get_channel(priv->wdev->wiphy, ieee80211_channel_to_frequency(data[1]
...
#define CMD_BUF_LEN   2048
    ...
    if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
        ...
        /* Check command */
        if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_VERSION, strlen(PRIV_CMD_VERSION)) == 0) {
            /* Get version */
            len =  woal_get_priv_driver_version(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_BANDCFG, strlen(PRIV_CMD_BANDCFG)) == 0) {
            /* Set/Get band configuration */
            len =  woal_setget_priv_bandcfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_HOSTCMD, strlen(PRIV_CMD_HOSTCMD)) == 0) {
            /* hostcmd configuration */
            len =  woal_priv_hostcmd(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_HTTXCFG, strlen(PRIV_CMD_HTTXCFG)) == 0) {
            /* Set/Get HT Tx configuration */
            len =  woal_setget_priv_httxcfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_HTCAPINFO, strlen(PRIV_CMD_HTCAPINFO)) == 0) {
            /* Set/Get HT Capability information */
            len =  woal_setget_priv_htcapinfo(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_ADDBAPARA, strlen(PRIV_CMD_ADDBAPARA)) == 0) {
            /* Set/Get Add BA parameters */
            len =  woal_setget_priv_addbapara(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_AGGRPRIOTBL, strlen(PRIV_CMD_AGGRPRIOTBL)) == 0) {
            /* Set/Get Aggregation priority table parameters */
            len =  woal_setget_priv_aggrpriotbl(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_ADDBAREJECT, strlen(PRIV_CMD_ADDBAREJECT)) == 0) {
            /* Set/Get Add BA reject parameters */
            len =  woal_setget_priv_addbareject(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_DELBA, strlen(PRIV_CMD_DELBA)) == 0) {
            /* Delete selective BA based on parameters */
            len =  woal_priv_delba(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_REJECTADDBAREQ, strlen(PRIV_CMD_REJECTADDBAREQ)) == 0) {
            /* Set/Get the reject addba requst conditions*/
            len =  woal_priv_rejectaddbareq(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_DATARATE, strlen(PRIV_CMD_DATARATE)) == 0) {
            /* Get data rate */
            len =  woal_get_priv_datarate(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_TXRATECFG, strlen(PRIV_CMD_TXRATECFG)) == 0) {
            /* Set/Get tx rate cfg */
            len =  woal_setget_priv_txratecfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_CUSTOMIE, strlen(PRIV_CMD_CUSTOMIE)) == 0) {
            /* Custom IE configuration */
            len =  woal_priv_customie(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_ESUPPMODE, strlen(PRIV_CMD_ESUPPMODE)) == 0) {
            /* Esupplicant mode configuration */
            len =  woal_setget_priv_esuppmode(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_PASSPHRASE, strlen(PRIV_CMD_PASSPHRASE)) == 0) {
            /* Esupplicant passphrase configuration */
            len =  woal_setget_priv_passphrase(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_DEAUTH, strlen(PRIV_CMD_DEAUTH)) == 0) {
            /* Deauth */
            len =  woal_priv_deauth(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef UAP_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_AP_DEAUTH, strlen(PRIV_CMD_AP_DEAUTH)) == 0) {
            /* AP Deauth */
            len =  woal_priv_ap_deauth(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_GET_STA_LIST, strlen(PRIV_CMD_GET_STA_LIST)) == 0) {
            /* Get STA list */
            len =  woal_priv_get_sta_list(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_BSS_CONFIG, strlen(PRIV_CMD_BSS_CONFIG)) == 0) {
            /* BSS config */
...
#if defined(WIFI_DIRECT_SUPPORT)
#if defined(STA_SUPPORT) && defined(UAP_SUPPORT)
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_BSSROLE, strlen(PRIV_CMD_BSSROLE)) == 0) {
            /* BSS Role */
...
#endif
#ifdef STA_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SETUSERSCAN, strlen(PRIV_CMD_SETUSERSCAN)) == 0) {
            /* Set user scan */
            len =  woal_priv_setuserscan(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_GETSCANTABLE, strlen(PRIV_CMD_GETSCANTABLE)) == 0) {
            /* Get scan table */
            len =  woal_priv_getscantable(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_EXTCAPCFG, strlen(PRIV_CMD_EXTCAPCFG)) == 0) {
            /* Extended capabilities configure */
            len =  woal_priv_extcapcfg(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_DEEPSLEEP, strlen(PRIV_CMD_DEEPSLEEP)) == 0) {
            /* Deep sleep */
            len =  woal_priv_setgetdeepsleep(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_IPADDR, strlen(PRIV_CMD_IPADDR)) == 0) {
            /* IP address */
            len =  woal_priv_setgetipaddr(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_WPSSESSION, strlen(PRIV_CMD_WPSSESSION)) == 0) {
            /* WPS Session */
            len =  woal_priv_setwpssession(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_OTPUSERDATA, strlen(PRIV_CMD_OTPUSERDATA)) == 0) {
            /* OTP user data */
            len =  woal_priv_otpuserdata(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_COUNTRYCODE, strlen(PRIV_CMD_COUNTRYCODE)) == 0) {
            /* Country code */
            len =  woal_priv_set_get_countrycode(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_TCPACKENH, strlen(PRIV_CMD_TCPACKENH)) == 0) {
            /* TCP ack enhancement */
            len =  woal_priv_setgettcpackenh(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef REASSOCIATION
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_ASSOCESSID, strlen(PRIV_CMD_ASSOCESSID)) == 0) {
            /* Associate to essid */
            len =  woal_priv_assocessid(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
         } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_WAKEUPREASON, strlen(PRIV_CMD_WAKEUPREASON)) == 0) {
            /* wakeup reason */
            len =  woal_priv_getwakeupreason(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef STA_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_LISTENINTERVAL, strlen(PRIV_CMD_LISTENINTERVAL)) == 0) {
            /* Listen Interval */
            len =  woal_priv_set_get_listeninterval(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
#ifdef DEBUG_LEVEL1
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_DRVDBG, strlen(PRIV_CMD_DRVDBG)) == 0) {
            /* Driver debug bit mask */
            len =  woal_priv_set_get_drvdbg(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_HSCFG, strlen(PRIV_CMD_HSCFG)) == 0) {
            /* HS configuration */
            len =  woal_priv_hscfg(priv, buf, priv_cmd.total_len, MTRUE);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_HSSETPARA, strlen(PRIV_CMD_HSSETPARA)) == 0) {
            /* Set HS parameter */
            len =  woal_priv_hssetpara(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SCANCFG, strlen(PRIV_CMD_SCANCFG)) == 0) {
            /* Scan configuration */
            len =  woal_priv_set_get_scancfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SET_BSS_MODE, strlen(PRIV_CMD_SET_BSS_MODE)) == 0) {
            /* Set bss mode */
            len = woal_priv_set_bss_mode(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef STA_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SET_AP, strlen(PRIV_CMD_SET_AP)) == 0) {
            /* Set AP */
            len = woal_priv_set_ap(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SET_POWER, strlen(PRIV_CMD_SET_POWER)) == 0) {
            /* Set power management parameters */
            len = woal_priv_set_power(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SET_ESSID, strlen(PRIV_CMD_SET_ESSID)) == 0) {
            /* Set essid */
            len = woal_priv_set_essid(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SET_AUTH, strlen(PRIV_CMD_SET_AUTH)) == 0) {
            /* Set authentication mode parameters */
            len = woal_priv_set_auth(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_GET_AP, strlen(PRIV_CMD_GET_AP)) == 0) {
            /* Get AP */
            len = woal_priv_get_ap(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_GET_POWER, strlen(PRIV_CMD_GET_POWER)) == 0) {
            /* Get power management parameters */
            len = woal_priv_get_power(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_WARMRESET, strlen(PRIV_CMD_WARMRESET)) == 0) {
            /* Performs warm reset */
            len = woal_priv_warmreset(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_TXPOWERCFG, strlen(PRIV_CMD_TXPOWERCFG)) == 0) {
            /* TX power configurations */
            len = woal_priv_txpowercfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_PSCFG, strlen(PRIV_CMD_PSCFG)) == 0) {
            /* PS configurations */
            len = woal_priv_pscfg(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SLEEPPD, strlen(PRIV_CMD_SLEEPPD)) == 0) {
            /* Sleep period */
            len = woal_priv_sleeppd(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_TXCONTROL, strlen(PRIV_CMD_TXCONTROL)) == 0) {
            /* Tx control */
            len = woal_priv_txcontrol(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_REGRDWR, strlen(PRIV_CMD_REGRDWR)) == 0) {
            /* Register Read/Write */
            len = woal_priv_regrdwr(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_RDEEPROM, strlen(PRIV_CMD_RDEEPROM)) == 0) {
            /* Read the EEPROM contents of the card */
            len = woal_priv_rdeeprom(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_MEMRDWR, strlen(PRIV_CMD_MEMRDWR)) == 0) {
            /* Memory Read/Write */
            len = woal_priv_memrdwr(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_SDCMD52RW, strlen(PRIV_CMD_SDCMD52RW)) == 0) {
            /* Cmd52 read/write register */
            len = woal_priv_sdcmd52rw(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef STA_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_ARPFILTER, strlen(PRIV_CMD_ARPFILTER)) == 0) {
            /* ARPFilter Configuration */
            len =  woal_priv_arpfilter(priv, buf, priv_cmd.total_len);
            goto handled;
#endif
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_MGMT_FRAME_CTRL, strlen(PRIV_CMD_MGMT_FRAME_CTRL)) == 0) {
            /* Mgmt Frame Passthrough Ctrl */
            len =  woal_priv_mgmt_frame_passthru_ctrl(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_QCONFIG, strlen(PRIV_CMD_QCONFIG)) == 0) {
            /* Queue config */
            len =  woal_priv_qconfig(priv, buf, priv_cmd.total_len);
            goto handled;
#ifdef STA_SUPPORT
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_QOS_CFG, strlen(PRIV_CMD_QOS_CFG)) == 0) {
            ...
            goto handled;
#endif
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_MAC_CTRL, strlen(PRIV_CMD_MAC_CTRL)) == 0) {
            /* MAC CTRL */
            len = woal_priv_macctrl(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_GETWAP, strlen(PRIV_CMD_GETWAP)) == 0) {
            /* Get WAP */
            len =  woal_priv_getwap(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_REGION_CODE, strlen(PRIV_CMD_REGION_CODE)) == 0) {
            /* Region Code */
            len = woal_priv_region_code(priv, buf, priv_cmd.total_len);
            goto handled;
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_FWMACADDR, strlen(PRIV_CMD_FWMACADDR)) == 0) {
            /* Set FW MAC address */
...
#ifdef STA_CFG80211
#if LINUX_VERSION_CODE >= WIFI_DIRECT_KERNEL_VERSION
        } else if (strnicmp(buf + strlen(CMD_MARVELL), PRIV_CMD_OFFCHANNEL, strlen(PRIV_CMD_OFFCHANNEL)) == 0) {
    ...
    if(strncmp(buf,"RSSILOW-THRESHOLD",strlen("RSSILOW-THRESHOLD")) == 0) {
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_rssi_low_threshold(priv, pdata,MOAL_IOCTL_WAIT)) {
            ret = - EFAULT;
            goto done;
        }
        len = sprintf(buf,"OK\n") + 1;
        ...
        if (MLAN_STATUS_SUCCESS != woal_set_scan_cfg(priv, buf, priv_cmd.total_len)) {
            ret = -EFAULT;
            goto done;
        }
        len = sprintf(buf, "OK\n") + 1;
    ...
    if(strncmp(buf,"MACADDR",strlen("MACADDR")) == 0) {
        ...
        if((ret = woal_uap_set_ap_cfg(priv, pdata,priv_cmd.used_len - strlen("AP_SET_CFG") - 1)))
            goto done;
        len = sprintf(buf,"OK\n") + 1;
    }
	...
	/* it will be done by GUI */
    ...
    else if(strncmp(buf,"BTCOEXSCAN-START",strlen("BTCOEXSCAN-START")) == 0) {
        len = sprintf(buf,"OK\n") + 1;
    }
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_bg_scan(priv, buf, priv_cmd.total_len)) {
            ret = - EFAULT;
            goto done;
        }
        ...
        if(MLAN_STATUS_SUCCESS != woal_set_ap_wps_p2p_ie(priv, (t_u8 *)pdata,
                    priv_cmd.used_len - strlen("SET_AP_WPS_P2P_IE") - 1)) {
            ret = - EFAULT;
            goto done;
        }
#endif
...
handled:
    ...
    if (len > 0) {
        ...
        if (priv_cmd.used_len <= priv_cmd.total_len) {
            memset(priv_cmd.buf + priv_cmd.used_len, 0, priv_cmd.total_len - priv_cmd.used_len);
            ...
            if (copy_to_user(req->ifr_data, &priv_cmd, sizeof(android_wifi_priv_cmd))) {
                PRINTM(MERROR, "%s: failed to copy command header to user buffer\n",
                       __FUNCTION__);
                ret = -EFAULT;
            }
        } else {
            PRINTM(MERROR, "%s: the buffer supplied by appl is too small (supplied: %d, used: %d)\n",
                                               ...
                                               BSSDescriptor_t* pbss_desc)
{
    ...
    memcpy(ptmp_buf, &pbss_desc->beacon_period, sizeof(pbss_desc->beacon_period));
    ...
    if (pbss_desc->pwpa_ie) {
        if ((*(pbss_desc->pwpa_ie)).vend_hdr.element_id == WPA_IE) {
            ie_len = sizeof(IEEEtypes_Header_t) + (*(pbss_desc->pwpa_ie)).vend_hdr.len;
    ...
    if (pbss_desc->prsn_ie) {
        if ((*(pbss_desc->prsn_ie)).ieee_hdr.element_id == RSN_IE) {
            ie_len = sizeof(IEEEtypes_Header_t) + (*(pbss_desc->prsn_ie)).ieee_hdr.len;
 ...
 */
int wlan_get_scan_table_ret_entry(BSSDescriptor_t* pbss_desc,
                                         t_u8** ppbuffer,
                                         int* pspace_left)
{
