 ...
 *  @return        N/A
 */
t_void wlan_reset_connect_state(
    pmlan_private priv,
    t_u8          drv_disconnect)
{
...
t_void wlan_2040_coex_event(pmlan_private pmpriv)
{
    ...
    if (pmpriv->curr_bss_params.bss_descriptor.poverlap_bss_scan_param &&
        pmpriv->curr_bss_params.bss_descriptor.poverlap_bss_scan_param->ieee_hdr.element_id == OVERLAPBSSSCANPARAM) {
        ...
        memcpy(pmpriv->adapter, (t_u8 *)pevent->event_buf,
               (t_u8 *)&pmpriv->curr_bss_params.bss_descriptor.poverlap_bss_scan_param->obss_scan_param, ele_len
              );
...
mlan_status wlan_ops_sta_process_event(IN t_void * priv)
{
        ...
        case EVENT_DEAUTHENTICATED:
            reason_code = *(t_u16 *)(pmbuf->pbuf + pmbuf->data_offset + sizeof(eventcause));
        ...
        case EVENT_SAD_REPORT:
                {
                    ...
                    PRINTM(MEVENT, "EVENT: Antenna Diversity %d  (%d, %d, %d, %d)\n",
                           eventcause, pEvtDat[0]+1, pEvtDat[1]+1, pEvtDat[2], pEvtDat[3]);
                }
                break;
