...
static mlan_status wlan_add_bsspriotbl(pmlan_private priv)
{
	...
	util_enqueue_list_tail(pmadapter->pmoal_handle, &pmadapter->bssprio_tbl[priv->bss_priority].
...
static t_void wlan_delete_bsspriotbl(pmlan_private priv)
{
    ...
    for (i = 0; i < pmadapter->priv_num; ++i) {
        ...
        if (*ppcur) {
            pbssprio_node = (mlan_bssprio_node *) util_peek_list(pmadapter->pmoal_handle,
                            phead, pmadapter->callbacks.moal_spin_lock,
                            pmadapter->callbacks.moal_spin_unlock);
...
mlan_status wlan_allocate_adapter(pmlan_adapter pmadapter)
{
    ...
    if (pmadapter->callbacks.moal_vmalloc && pmadapter->callbacks.moal_vfree)
        ret = pmadapter->callbacks.moal_vmalloc(pmadapter->pmoal_handle, buf_size, (t_u8 **)&ptemp_scan_table);
    else
    ...
    ret = pmadapter->callbacks.moal_malloc(pmadapter->pmoal_handle, MAX_MP_REGS + DMA_ALIGNMENT, MLAN_MEM_DEF | MLAN_MEM_DMA, (t_u8 **)&pmadapter->mp_regs_buf);
    ...
    pmadapter->psleep_cfm = wlan_alloc_mlan_buffer(pmadapter, sizeof(opt_sleep_confirm_buffer),0,MOAL_MALLOC_BUFFER);
...
mlan_status wlan_init_lock_list (
    ...
    for (i = 0; i < pmadapter->priv_num; i++) {
        if (pmadapter->priv[i]) {
            ...
            util_init_list_head((t_void *)pmadapter->pmoal_handle, &priv->bypass_txq,
...
t_void wlan_free_lock_list (
    ...
    util_free_list_head((t_void *)pmadapter->pmoal_handle,
                        &pmadapter->cmd_free_q, pmadapter->callbacks.moal_free_lock);

    util_free_list_head((t_void *)pmadapter->pmoal_handle,
                        &pmadapter->cmd_pending_q, pmadapter->callbacks.moal_free_lock);

    util_free_list_head((t_void *)pmadapter->pmoal_handle,
                        &pmadapter->scan_pending_q, pmadapter->callbacks.moal_free_lock);
    ...
    for (i = 0; i < pmadapter->priv_num; i++) {
        if (pmadapter->priv[i]) {
            priv = pmadapter->priv[i];
            util_free_list_head((t_void *)pmadapter->pmoal_handle,
                                &priv->sta_list,priv->adapter->callbacks.moal_free_lock);
            util_free_list_head((t_void *)pmadapter->pmoal_handle,
                        &priv->bypass_txq,pmadapter->callbacks.moal_free_lock);
