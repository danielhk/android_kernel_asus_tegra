...
void wlan_notify_station_deauth(mlan_private *priv)
{
    ...
    while (sta_ptr != (sta_node *) &priv->sta_list) {
        ...
        memcpy(priv->adapter, (t_u8 *)pevent->event_buf+2,sta_ptr->mac_addr,MLAN_MAC_ADDR_LENGTH);
 ...
 */
static mlan_status wlan_uap_cmd_802_11_hs_cfg(
    ...
    IN hs_config_param      *pdata_buf)
{
	...
	if(pmpriv->adapter->min_wake_holdoff){
		...
		holdoff_tlv->header.len = wlan_cpu_to_le16(sizeof(MrvlIEtypes_HsWakeHoldoff_t) - sizeof(MrvlIEtypesHeader_t));
 ...
 *  @return         MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_uap_cmd_txdatapause(
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_uap_ret_txdatapause(
    IN pmlan_private        pmpriv,
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t)){
        ...
        if(tlv_type == TLV_TYPE_TX_PAUSE) {
            ...
            PRINTM(MCMND,"TxPause: %02x:%02x:%02x:%02x:%02x:%02x pause=%d, pkts=%d\n",
                    tx_pause_tlv->peermac[0],tx_pause_tlv->peermac[1],tx_pause_tlv->peermac[2],
                    tx_pause_tlv->peermac[3],tx_pause_tlv->peermac[4],tx_pause_tlv->peermac[5],
 ...
 */
static mlan_status wlan_uap_cmd_ap_config(
    pmlan_private           pmpriv,
    IN HostCmd_DS_COMMAND   *cmd,
    IN t_u16                cmd_action,
    IN pmlan_ioctl_req      pioctl_buf)
{
    ...
    if(bss->param.bss_config.groupwise_update_timeout < (MAX_VALID_DWORD)) {
        ...
        tlv_groupwise_timeout->groupwise_update_timeout = wlan_cpu_to_le32(bss->param.bss_config.groupwise_update_timeout);
    ...
    if((bss->param.bss_config.filter.filter_mode <= MAC_FILTER_MODE_BLOCK_MAC)
        ...
        tlv_mac_filter->header.len = wlan_cpu_to_le16(2+ MLAN_MAC_ADDR_LENGTH *
        ...
        cmd_size += sizeof(MrvlIEtypesHeader_t) + 2 + MLAN_MAC_ADDR_LENGTH *
                    bss->param.bss_config.filter.mac_count;
        tlv += sizeof(MrvlIEtypesHeader_t) + 2 + MLAN_MAC_ADDR_LENGTH *
                    bss->param.bss_config.filter.mac_count;
    }
    ...
    if((bss->param.bss_config.num_of_chan) &&
        ...
        tlv_chan_list->header.len = wlan_cpu_to_le16((t_u16)(sizeof(ChanScanParamSet_t) *
    ...
    if(bss->param.bss_config.wmm_para.qos_info == 0x80 || bss->param.bss_config.wmm_para.qos_info == 0x00) {
        ...
        memcpy(pmpriv->adapter, tlv_wmm_parameter->wmm_para.ouitype, bss->param.bss_config.wmm_para.ouitype, sizeof(tlv_wmm_parameter->wmm_para.ouitype));
        ...
        for (ac = 0; ac < 4; ac++) {
            tlv_wmm_parameter->wmm_para.ac_params[ac].aci_aifsn.aifsn = bss->param.bss_config.wmm_para.ac_params[ac].aci_aifsn.aifsn;
            tlv_wmm_parameter->wmm_para.ac_params[ac].aci_aifsn.aci = bss->param.bss_config.wmm_para.ac_params[ac].aci_aifsn.aci;
            tlv_wmm_parameter->wmm_para.ac_params[ac].ecw.ecw_max = bss->param.bss_config.wmm_para.ac_params[ac].ecw.ecw_max;
            tlv_wmm_parameter->wmm_para.ac_params[ac].ecw.ecw_min = bss->param.bss_config.wmm_para.ac_params[ac].ecw.ecw_min;
            tlv_wmm_parameter->wmm_para.ac_params[ac].tx_op_limit = wlan_cpu_to_le16(bss->param.bss_config.wmm_para.ac_params[ac].tx_op_limit);
        }
 ...
 *  @return         MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status wlan_uap_cmd_sys_configure(
    pmlan_private           pmpriv,
    IN HostCmd_DS_COMMAND   *cmd,
    IN t_u16                cmd_action,
    IN pmlan_ioctl_req      pioctl_buf,
    ...
    if(pioctl_buf == MNULL) {
        if (pdata_buf) {
            ...
            case TLV_TYPE_UAP_CHAN_BAND_CONFIG:
                ...
                cmd->size = wlan_cpu_to_le16(sizeof(HostCmd_DS_SYS_CONFIG) - 1
                                + S_DS_GEN + sizeof(MrvlIEtypes_channel_band_t));
                chan_band_tlv->header.type = wlan_cpu_to_le16(TLV_TYPE_UAP_CHAN_BAND_CONFIG);
                chan_band_tlv->header.len= wlan_cpu_to_le16(sizeof(MrvlIEtypes_channel_band_t)
            ...
            case TLV_TYPE_UAP_BEACON_PERIOD:
                ...
                bcn_pd_tlv->header.len= wlan_cpu_to_le16(sizeof(MrvlIEtypes_beacon_period_t)
                                                         - sizeof(MrvlIEtypesHeader_t));
                if (cmd_action) {
                ...
                if (TLV_TYPE_UAP_DTIM_PERIOD == pdat_tlv_dtimpd->header.type) {
                    ...
                    dtim_pd_tlv->header.len= wlan_cpu_to_le16(sizeof(MrvlIEtypes_dtim_period_t)
                                                              - sizeof(MrvlIEtypesHeader_t));
                    if (cmd_action) {
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_uap_ret_cmd_ap_config(
    IN pmlan_private        pmpriv,
    IN HostCmd_DS_COMMAND   *resp,
    IN mlan_ioctl_req       *pioctl_buf)
{
    ...
    while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
        ...
        switch(tlv_type) {
            ...
            case TLV_TYPE_SSID:
                ...
                memcpy(pmpriv->adapter, bss->param.bss_config.ssid.ssid, tlv_ssid->ssid, MIN(MLAN_MAX_SSID_LENGTH,tlv_len));
                break;
            case TLV_TYPE_UAP_BEACON_PERIOD:
                tlv_beacon_period = (MrvlIEtypes_beacon_period_t *)tlv;
            ...
            case TLV_TYPE_UAP_EAPOL_PWK_HSK_TIMEOUT:
                tlv_pairwise_timeout = (MrvlIEtypes_eapol_pwk_hsk_timeout_t *)tlv;
                bss->param.bss_config.pairwise_update_timeout = wlan_le32_to_cpu(tlv_pairwise_timeout->pairwise_update_timeout);
                break;
            ...
            case TLV_TYPE_UAP_EAPOL_GWK_HSK_TIMEOUT:
                tlv_groupwise_timeout = (MrvlIEtypes_eapol_gwk_hsk_timeout_t *)tlv;
                bss->param.bss_config.groupwise_update_timeout = wlan_le32_to_cpu(tlv_groupwise_timeout->groupwise_update_timeout);
                break;
            ...
            case TLV_TYPE_VENDOR_SPECIFIC_IE:
                ...
                for (ac = 0; ac < 4; ac++) {
                    bss->param.bss_config.wmm_para.ac_params[ac].aci_aifsn.aifsn = tlv_wmm_parameter->wmm_para.ac_params[ac].aci_aifsn.aifsn;
                    bss->param.bss_config.wmm_para.ac_params[ac].aci_aifsn.aci = tlv_wmm_parameter->wmm_para.ac_params[ac].aci_aifsn.aci;
                    bss->param.bss_config.wmm_para.ac_params[ac].ecw.ecw_max = tlv_wmm_parameter->wmm_para.ac_params[ac].ecw.ecw_max;
                    bss->param.bss_config.wmm_para.ac_params[ac].ecw.ecw_min = tlv_wmm_parameter->wmm_para.ac_params[ac].ecw.ecw_min;
                    bss->param.bss_config.wmm_para.ac_params[ac].tx_op_limit = wlan_le16_to_cpu(tlv_wmm_parameter->wmm_para.ac_params[ac].tx_op_limit);
                }
                break;
        }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_uap_ret_sys_reset(
    IN pmlan_private        pmpriv,
    IN HostCmd_DS_COMMAND   *resp,
    IN mlan_ioctl_req       *pioctl_buf)
{
                ...
                if (max_mgmt_ie) {
                    ...
                    if (max_mgmt_ie->type == TLV_TYPE_MAX_MGMT_IE) {
                        ...
                        memcpy(pmpriv->adapter, (t_u8 *)&misc->param.cust_ie + (cust_ie->len + sizeof(MrvlIEtypesHeader_t)),
                            max_mgmt_ie, MIN(sizeof(tlvbuf_max_mgmt_ie), max_mgmt_ie->len + sizeof(MrvlIEtypesHeader_t)));
                    }
                }
            }
        }
 ...
 *  @return         MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status wlan_uap_cmd_snmp_mib(
    pmlan_private           pmpriv,
    IN HostCmd_DS_COMMAND   *cmd,
    IN t_u16                cmd_action,
    IN t_u32                cmd_oid,
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_uap_ret_snmp_mib(
    IN pmlan_private        pmpriv,
    IN HostCmd_DS_COMMAND   *resp,
    IN mlan_ioctl_req       *pioctl_buf)
{
    ...
    IN t_void               *pdata_buf)
{
    ...
    if (pkey->is_wapi_key) {
        ...
        if (0 != memcmp(pmpriv->adapter, pkey->mac_addr, bc_mac, sizeof(bc_mac))) { /* WAPI pairwise key: unicast */
...
static void wlan_check_sta_capability(pmlan_private priv, pmlan_buffer pevent, sta_node *sta_ptr)
{
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t)){
        ...
        if(tlv_type == TLV_TYPE_UAP_MGMT_FRAME) {
            ...
            memcpy(priv->adapter, &frame_control, (t_u8 *)&(mgmt_tlv->frame_control),
            ...
            if((mgmt_tlv->frame_control.type == 0) &&
                ...
                sta_ptr->is_wmm_enabled = wlan_is_wmm_ie_present(priv->adapter, assoc_req_ie, ie_len);
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t)){
        ...
        if(tlv_type == VENDOR_SPECIFIC_221) {
            if (!memcmp(priv->adapter, (t_u8 *)tlv + sizeof(MrvlIEtypesHeader_t), wmm_oui, sizeof(wmm_oui))) {
                ...
                memcpy(priv->adapter, &WmmParamIe, ((t_u8 *)tlv + 2), sizeof(IEEEtypes_WmmParameter_t));
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ops_uap_prepare_cmd(
    IN t_void		        *priv,
    IN t_u16                cmd_no,
    IN t_u16                cmd_action,
    IN t_u32                cmd_oid,
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ops_uap_process_cmdresp(
    IN t_void               *priv,
    IN t_u16                cmdresp_no,
    IN t_void               *pcmd_buf,
    IN t_void               *pioctl
...
mlan_status wlan_ops_uap_process_event(IN t_void * priv)
{
    ...
    switch(eventcause) {
        ...
        case EVENT_BA_STREAM_TIMEOUT:
                ...
                if (pmpriv->media_connected == MTRUE)
                    wlan_11n_ba_stream_timeout(pmpriv,
                        (HostCmd_DS_11N_BATIMEOUT *) pmadapter->event_body);
                else
	    ...
	    case EVENT_AMSDU_AGGR_CTRL:
                ...
                pmadapter->tx_buf_size = MIN(pmadapter->curr_tx_buf_size
 ...
 */
mlan_status wlan_ops_uap_init_cmd (
    IN  t_void *  priv,
    IN  t_u8      first_bss)
{
    mlan_status     ret = MLAN_STATUS_SUCCESS;
