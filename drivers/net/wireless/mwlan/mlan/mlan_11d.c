 ...
 */
static t_u8 wlan_11d_channel_known(
 ...
 */
static mlan_status wlan_11d_generate_domain_info(
    pmlan_adapter pmadapter,
    parsed_region_chan_11d_t   *parsed_region_chan)
{
 ...
 */
static mlan_status wlan_11d_update_chan_pwr_table(
    mlan_private *pmpriv,
    BSSDescriptor_t *pbss_desc)
{
    ...
    /* Check whether the channel already exists in channel power table of parsed region */
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status wlan_11d_process_country_info(
    mlan_private *pmpriv,
    BSSDescriptor_t *pbss_desc)
{
    mlan_adapter  *pmadapter = pmpriv->adapter;
    ...
    if (parsed_region_chan->no_of_chan != 0) {
        ...
        for (i = 0; (i < region_chan.no_of_chan && i < MAX_NO_OF_CHAN); i++) {
            ...
            if (j == parsed_region_chan->no_of_chan && j < MAX_NO_OF_CHAN) {
                 ...
                 */
                parsed_region_chan->chan_pwr[parsed_region_chan->no_of_chan + num_chan_added].chan = region_chan.chan_pwr[i].chan;
                parsed_region_chan->chan_pwr[parsed_region_chan->no_of_chan + num_chan_added].band = region_chan.chan_pwr[i].band;
                parsed_region_chan->chan_pwr[parsed_region_chan->no_of_chan + num_chan_added].pwr = region_chan.chan_pwr[i].pwr;
 ...
 *  @return           N/A
 */
static t_void wlan_11d_copy_chan_power(
    chan_power_11d_t *chan_dst,
 ...
 *  @return                     N/A
 */
static t_void wlan_11d_sort_parsed_region_chan(
    parsed_region_chan_11d_t  *parsed_region_chan)
{
    int i, j;
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11d_enable(mlan_private  *pmpriv,
                            t_void        *pioctl_buf,
                            state_11d_t   flag)
{
#ifdef STA_SUPPORT
    ...
    parsed_region_chan_11d_t       *parsed_region_chan)
{
    ...
    for (j = 0, last_chan = 0; j < no_of_sub_band; j++) {
        ...
        for (i = 0; idx < MAX_NO_OF_CHAN && i < no_of_chan; i++ ) {
            ...
            /* Step 5: We don't need to check if cur_chan is supported by mrvl in region */
 ...
 */
t_u8 wlan_11d_get_scan_type(pmlan_adapter pmadapter,
                            t_u8  band,
                            t_u8  chan,
                            parsed_region_chan_11d_t *parsed_region_chan)
{
    ...
    IN mlan_ioctl_req   *pioctl_req)
{
    ...
    if (wlan_misc_country_2_cfp_table_code(pmadapter,
            domain_info->country_code, &cfp_bg, &cfp_a)) {
    ...
    memcpy(pmadapter, pmadapter->country_code,domain_info->country_code, COUNTRY_CODE_LEN);
    ...
    t_void                 *pioctl_buf)
{
    ...
    if (wlan_misc_country_2_cfp_table_code(pmadapter,
            pdomain_tlv->country_code, &cfp_bg, &cfp_a)) {
        PRINTM(MERROR, "Country code not found!\n");
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    ...
    memcpy(pmadapter, pmadapter->country_code,pdomain_tlv->country_code, COUNTRY_CODE_LEN);
