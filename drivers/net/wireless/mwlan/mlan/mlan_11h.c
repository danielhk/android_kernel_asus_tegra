 ...
 */
static
t_u16 wlan_11h_set_supp_channels_ie( mlan_private *priv,
                                   t_u8 band,
                                             ...
                                             const t_void *pinfo_buf )
{
    ...
    PRINTM(MINFO, "11h: ChSwAnn: Ch=%d, Cnt=%d, Mode=%d\n",
           pch_sw_ann->new_chan, pch_sw_ann->switch_count, pch_sw_ann->switch_mode );
 ...
 */
static t_u32 wlan_11h_set_local_power_constraint_tlv( t_u8** ppbuffer,
                                                    t_u8 channel,
                                                    ...
                                                    t_u8 max_tx_power_capability)
{
    ...
    PRINTM(MINFO, "11h: Set local power constraint = %d channel=%d min_tx_pwr=%d max_tx_pwr=%d\n",
           power_constraint,channel,
           min_tx_power_capability,max_tx_power_capability);
 ...
 *                       buffer (ppbuffer)
 */
static t_u32 wlan_11h_process_infra_join( mlan_private *priv,
                                          t_u8** ppbuffer,
                                          t_u8  band,
                                          t_u32 channel,
 ...
 *  @return MTRUE or MFALSE
 */
static t_bool wlan_only_uap_priv_in_list(mlan_private **priv_list,
 ...
 */
static wlan_dfs_timestamp_t* wlan_11h_find_dfs_timestamp(mlan_adapter* pmadapter,
                                                         t_u8 channel)
{
    ...
    while (pts && pts != (wlan_dfs_timestamp_t*)&pmadapter->state_dfs.dfs_ts_head) {
        PRINTM(MINFO, "dfs_timestamp(@ %p) - chan=%d, repr=%d(%s),"
               " time(sec.usec)=%lu.%06lu\n", pts, pts->channel, pts->represents,
               DFS_TS_REPR_STRINGS[pts->represents], pts->ts_sec, pts->ts_usec);
                                              ...
                                              t_u8 repr, t_u8 channel)
{
    ...
    if (!pdfs_ts) {
        ...
        ret = pmadapter->callbacks.moal_malloc(pmadapter->pmoal_handle,
                sizeof(wlan_dfs_timestamp_t), MLAN_MEM_DEF, (t_u8**)&pdfs_ts);
 ...
 *  @return      MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_activate(mlan_private *priv,
                              t_void       *pioctl_buf,
                              t_bool       flag)
{
...
t_u8 wlan_11h_get_adhoc_start_channel( mlan_private *priv )
{
    ...
    PRINTM(MINFO, "11h: %s: AdHoc Channel set to %u\n",
           wlan_11h_is_enabled(priv) ? "Enabled" : "Disabled",
           start_chn);
...
t_u8 wlan_11h_get_csa_closed_channel(mlan_private *priv)
{
    ...
    if (sec > priv->intf_state_11h.dfs_slave_csa_expire_at_sec) {
        ...
        wlan_set_chan_blacklist(priv, BAND_A,
                        priv->intf_state_11h.dfs_slave_csa_chan, MFALSE);
 ...
 */
t_bool wlan_11h_radar_detect_required( mlan_private *priv,
                                       t_u8 channel )
{
                                       ...
                                       const t_u8 *pelement )
{
    ...
    case POWER_CONSTRAINT:
        ...
        p11h_bss_info->power_constraint.len = MIN(element_len,
    ...
    case POWER_CAPABILITY:
        ...
        p11h_bss_info->power_capability.len = MIN(element_len,
    ...
    case TPC_REPORT:
        ...
        p11h_bss_info->tpc_report.len = MIN(element_len,
    ...
    case CHANNEL_SWITCH_ANN:
        ...
        p11h_bss_info->chan_switch_ann.len = MIN(element_len,
    ...
    case QUIET:
        ...
        p11h_bss_info->quiet.len = MIN(element_len,
    ...
    case IBSS_DFS:
        ...
        p11h_bss_info->ibss_dfs.len = MIN(element_len,
 ...
 *  @return MTRUE or MFALSE
 */
t_bool wlan_11h_is_channel_under_nop(mlan_adapter* pmadapter,
                                     t_u8 channel)
{
    wlan_dfs_timestamp_t *pdfs_ts = MNULL;
...
mlan_status wlan_11h_radar_detected_handling(mlan_adapter* pmadapter)
{
        ...
        do {
            ...
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP)
                ...
                pstate_rdh->new_channel = wlan_11h_get_adhoc_start_channel(pmpriv);
                  ...
                  (++i < MAX_RANDOM_CHANNEL_RETRIES) );  /* avoid deadloop */
    ...
    case RDH_REM_CUSTOM_IE:
        ...
        /* remove CHAN_SW IE - firmware will accept on any interface, and apply to all */
        if (pstate_rdh->priv_curr_idx == RDH_STAGE_FIRST_ENTRY_PRIV_IDX) {
            mlan_ioctl_req *pioctl_req = MNULL;
    ...
    case RDH_SET_NEW_CHANNEL:
        ...
        while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count) {
            ...
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP) {
                ...
                pmpriv->uap_state_chan_cb.get_chan_callback
                                    = wlan_11h_radar_detected_callback;
                pstate_rdh->uap_band_cfg |= UAP_BAND_CONFIG_5GHZ;  /* DFS only in 5GHz */
...
#ifdef STA_SUPPORT
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_STA) {
                /* Check previous state to find former Ad-hoc creator interface.
                   Set new state to Starting, so it'll be seen as a DFS master. */
