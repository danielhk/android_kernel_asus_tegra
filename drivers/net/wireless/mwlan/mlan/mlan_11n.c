 ...
 */
static mlan_status wlan_11n_ioctl_max_tx_buf_size(
 ...
 *  @return				MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_htusrcfg(
	IN pmlan_adapter    pmadapter,
	IN pmlan_ioctl_req  pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 */
static mlan_status wlan_11n_ioctl_amsdu_aggr_ctrl(
	IN pmlan_adapter    pmadapter,
	IN pmlan_ioctl_req  pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_httxcfg(
	IN pmlan_adapter    pmadapter,
	IN pmlan_ioctl_req  pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 */
static mlan_status wlan_11n_ioctl_tx_bf_cap(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_tx_bf_cfg(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return             MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_stream_cfg(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
...
static void wlan_11n_update_addba_request(mlan_private *priv)
{
    ...
    if (!(ptx_tbl = (TxBAStreamTbl *) util_peek_list(
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_11n_ioctl_addba_param(
	IN pmlan_adapter    pmadapter,
	IN pmlan_ioctl_req  pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_addba_reject(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    int i = 0;
...
static mlan_status wlan_send_delba_to_entry_in_txbastream_tbl(
        pmlan_private priv, pmlan_ioctl_req pioctl_req,
        t_u8 tid, t_u8 *peer_address,
        TxBAStreamTbl *last_tx_ba_to_delete)
{
    ...
    if (!(tx_ba_stream_tbl_ptr = (TxBAStreamTbl *) util_peek_list(
        ...
        if (tx_ba_stream_tbl_ptr->ba_status == BA_STREAM_SETUP_COMPLETE) {
            if (((tid == DELBA_ALL_TIDS) || (tid == tx_ba_stream_tbl_ptr->tid)) &&
                ...
                if (last_tx_ba_to_delete && (tx_ba_stream_tbl_ptr == last_tx_ba_to_delete))
                    ret = wlan_send_delba(priv, pioctl_req, tx_ba_stream_tbl_ptr->tid, tx_ba_stream_tbl_ptr->ra, 1);
                else
                    ret = wlan_send_delba(priv, MNULL, tx_ba_stream_tbl_ptr->tid, tx_ba_stream_tbl_ptr->ra, 1);
            }
        }
        ...
        RxReorderTbl *last_rx_ba_to_delete)
{
    ...
    if (!(rx_reor_tbl_ptr = (RxReorderTbl *) util_peek_list(
                      pmadapter->pmoal_handle,
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_delba(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    ...
    PRINTM(MINFO, "DelBA: direction %d, TID %d, peer address %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    if (cfg->param.del_ba.direction & DELBA_TX) {
        if (last_rx_ba_to_delete)
            ...
            ret = wlan_send_delba_to_entry_in_txbastream_tbl(pmpriv, pioctl_req, tid, peer_address, last_tx_ba_to_delete);
    }
    if (last_rx_ba_to_delete) {
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_rejectaddbareq(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_aggr_prio_tbl(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    int i = 0;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_11n_ioctl_supported_mcs_set(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
    mlan_ds_11n_cfg *cfg= MNULL;
 ...
 *  @return             N/A
 */
static void wlan_fill_cap_info(mlan_private *priv,
 ...
 *  @return             N/A
 */
void wlan_fill_ht_cap_tlv(mlan_private *priv,
        MrvlIETypes_HTCap_t *pht_cap, t_u8 bands)
{
    mlan_adapter *pmadapter = priv->adapter;
    int rx_mcs_supp;
    ...
    if (pdel_ba->del_result == BA_RESULT_SUCCESS) {
              ...
              */
                if (INITIATOR_BIT(pdel_ba->del_ba_param_set)) {
                        ...
                        if ((ptx_ba_tbl = wlan_11n_get_txbastream_status(priv,
                                        BA_STREAM_SETUP_INPROGRESS))) {
                                mlan_11n_delete_bastream_tbl(priv, ptx_ba_tbl->tid, ptx_ba_tbl->ra,
 ...
 *  @return        MLAN_STATUS_SUCCESS
 */
mlan_status wlan_ret_amsdu_aggr_ctrl(
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_reject_addba_req(
    IN pmlan_private        pmpriv,
    IN HostCmd_DS_COMMAND   *cmd,
    IN t_u16                cmd_action,
    IN t_void               *pdata_buf)
 ...
 *  @return        MLAN_STATUS_SUCCESS
 */
mlan_status wlan_ret_reject_addba_req(
    IN  pmlan_private        pmpriv,
    IN  HostCmd_DS_COMMAND   *resp,
    IN  mlan_ioctl_req       *pioctl_buf)
{
    ...
    ENTER();
    if(pioctl_buf && (wlan_le16_to_cpu(pRejectAddbaReq->action) == HostCmd_ACT_GEN_GET)) {
        cfg = (mlan_ds_11n_cfg *)pioctl_buf->pbuf;
        cfg->param.reject_addba_req.conditions = wlan_le32_to_cpu(pRejectAddbaReq->conditions);
    }
    LEAVE();
    return MLAN_STATUS_SUCCESS;
}
 ...
 */
mlan_status wlan_cmd_tx_bf_cfg(
    IN pmlan_private        pmpriv,
    IN HostCmd_DS_COMMAND   *cmd,
    IN t_u16                cmd_action,
    IN t_void               *pdata_buf)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_SET) {
        ...
        case BF_GLOBAL_CONFIGURATION:
            ...
            txbfcfg->body.bf_global_cfg.sounding_interval = wlan_cpu_to_le16(txbf->body.bf_global_cfg.sounding_interval);
        ...
        case SET_GET_BF_PERIODICITY:
            txbfcfg->body.bf_periodicity.interval = wlan_cpu_to_le16(txbf->body.bf_periodicity->interval);
            break;
 ...
 *  @return        MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ret_tx_bf_cfg(
    IN  pmlan_private        pmpriv,
    IN  HostCmd_DS_COMMAND   *resp,
    IN  mlan_ioctl_req       *pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        case BF_GLOBAL_CONFIGURATION:
            ...
            txbf->body.bf_global_cfg.sounding_interval = wlan_le16_to_cpu(txbfcfg->body.bf_global_cfg.sounding_interval);
        ...
        case SET_GET_BF_PERIODICITY:
            ...
            txbf->body.bf_periodicity->interval = wlan_le16_to_cpu(txbfcfg->body.bf_periodicity.interval);
            break;
        case TX_BF_FOR_PEER_ENBL:
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
mlan_status wlan_11n_cfg_ioctl(
    IN pmlan_adapter    pmadapter,
    IN pmlan_ioctl_req  pioctl_req)
{
...
void wlan_11n_deleteall_txbastream_tbl(mlan_private *priv)
{
	...
	while ((del_tbl_ptr = (TxBAStreamTbl *)
			util_peek_list(priv->adapter->pmoal_handle, &priv->tx_ba_stream_tbl_ptr,
				priv->adapter->callbacks.moal_spin_lock,
		                priv->adapter->callbacks.moal_spin_unlock))) {
 ...
 *  @return	    N/A
 */
void wlan_11n_create_txbastream_tbl(mlan_private *priv,
 ...
 *  @return	        MLAN_STATUS_PENDING --success, otherwise fail
 */
int wlan_send_delba(mlan_private *priv, pmlan_ioctl_req pioctl_req, int tid, t_u8 *peer_mac, int initiator)
{
	...
	ret = wlan_prepare_cmd(priv, HostCmd_CMD_11N_DELBA,
					HostCmd_ACT_GEN_SET, 0, (t_void *)pioctl_req, (t_void *)&delba);
...
void wlan_11n_delete_bastream(mlan_private *priv, t_u8 *del_ba)
{
        ...
        mlan_11n_delete_bastream_tbl(priv, tid, pdel_ba->peer_mac_addr,
                TYPE_DELBA_RECEIVE, INITIATOR_BIT(pdel_ba->del_ba_param_set));
...
int wlan_get_rxreorder_tbl(mlan_private *priv, rx_reorder_tbl *buf)
{
    ...
    int count = 0;
    ENTER();
    if (!(rxReorderTblPtr = ( RxReorderTbl *) util_peek_list(priv->adapter->pmoal_handle,
            &priv->rx_reorder_tbl_ptr,
            priv->adapter->callbacks.moal_spin_lock,
            priv->adapter->callbacks.moal_spin_unlock))) {
        LEAVE();
...
int wlan_get_txbastream_tbl(mlan_private *priv,tx_ba_stream_tbl *buf)
{
    ...
    if (!(ptxtbl = (TxBAStreamTbl *) util_peek_list(
				priv->adapter->pmoal_handle,
				&priv->tx_ba_stream_tbl_ptr,
				priv->adapter->callbacks.moal_spin_lock,
	                        priv->adapter->callbacks.moal_spin_unlock))) {
