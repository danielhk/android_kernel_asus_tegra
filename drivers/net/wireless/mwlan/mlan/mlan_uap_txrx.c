...
static mlan_status wlan_upload_uap_rx_packet(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
	...
	pmadapter->callbacks.moal_get_system_time(
            pmadapter->pmoal_handle, &pmbuf->out_ts_sec, &pmbuf->out_ts_usec);
    PRINTM_NETINTF(MDATA, priv);
	PRINTM(MDATA, "%lu.%06lu : Data => kernel seq_num=%d tid=%d\n",
            pmbuf->out_ts_sec, pmbuf->out_ts_usec,prx_pd->seq_num, prx_pd->priority);
...
t_void * wlan_ops_uap_process_txpd(IN t_void * priv, IN pmlan_buffer pmbuf)
{
    ...
    head_ptr = pmbuf->pbuf + pmbuf->data_offset - sizeof(UapTxPD) - INTF_HEADER_LEN;
 ...
 *  @return	   MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ops_uap_process_rx_packet(
    IN t_void               *adapter,
    IN  pmlan_buffer        pmbuf)
{
	...
	if(pmadapter->priv[pmbuf->bss_index]->mgmt_frame_passthru_mask &&
        ...
        if ((puap_pkt_hdr->wlan_header.frm_ctl & IEEE80211_FC_MGMT_FRAME_TYPE_MASK) == 0)
	        wlan_process_802dot11_mgmt_pkt(pmadapter->priv[pmbuf->bss_index],
                    (t_u8 *)&puap_pkt_hdr->wlan_header, puap_pkt_hdr->frm_len
                     + sizeof(wlan_mgmt_pkt) - sizeof(puap_pkt_hdr->frm_len));
		wlan_free_mlan_buffer(pmadapter, pmbuf);
		goto done;
	}
    ...
    if(!priv->is_11n_enabled || (!wlan_11n_get_rxreorder_tbl((mlan_private *)priv, prx_pd->priority, ta)
	...
	if ((ret = mlan_11n_rxreorder_pkt(priv, prx_pd->seq_num,
		prx_pd->priority, ta, (t_u8)prx_pd->rx_pkt_type, (void *) pmbuf))
    ...
    IN  pmlan_buffer        pmbuf)
{
    ...
    PRINTM(MDATA, "AMSDU dest %02x:%02x:%02x:%02x:%02x:%02x\n", prx_pkt->eth803_hdr.dest_addr[0],
            prx_pkt->eth803_hdr.dest_addr[1],prx_pkt->eth803_hdr.dest_addr[2],
            prx_pkt->eth803_hdr.dest_addr[3],prx_pkt->eth803_hdr.dest_addr[4],
    ...
    if(prx_pkt->eth803_hdr.dest_addr[0] & 0x01) {
        ...
        /* Multicast pkt */
        if ((newbuf = wlan_alloc_mlan_buffer(pmadapter, MLAN_TX_DATA_BUF_SIZE_2K,0,MOAL_MALLOC_BUFFER))) {
            ...
            newbuf->data_offset = (sizeof(UapTxPD) + INTF_HEADER_LEN + DMA_ALIGNMENT);
        ...
        if ((!(priv->pkt_fwd & PKT_FWD_INTRA_UCAST)) && (wlan_get_station_entry(priv, prx_pkt->eth803_hdr.dest_addr))) {
            /* Intra BSS packet */
            if ((newbuf = wlan_alloc_mlan_buffer(pmadapter, MLAN_TX_DATA_BUF_SIZE_2K,0,MOAL_MALLOC_BUFFER))) {
                newbuf->bss_index = pmbuf->bss_index;
                newbuf->buf_type = pmbuf->buf_type;
        ...
        else if(MLAN_STATUS_FAILURE == wlan_check_unicast_packet(priv, prx_pkt->eth803_hdr.dest_addr)){
            /* drop packet */
    ...
    IN  pmlan_buffer        pmbuf)
{
    ...
    if((priv->media_connected == MFALSE) || ((pmbuf->data_len - prx_pd->rx_pkt_offset) > MV_ETH_FRAME_LEN))
        goto upload;
