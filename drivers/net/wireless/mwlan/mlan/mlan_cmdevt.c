...
void wlan_check_scan_queue(IN pmlan_adapter    pmadapter)
{
    ...
    if(!(pcmd_node = (cmd_ctrl_node *)util_peek_list(
...
t_void wlan_dump_info(mlan_adapter	*pmadapter, t_u8 reason)
{
    ...
    if(reason != REASON_CODE_CMD_TIMEOUT){
        if(pmadapter->data_sent || pmadapter->cmd_sent || (pmadapter->dbg.num_no_cmd_node >=5)
            || (pmadapter->pm_wakeup_card_req && pmadapter->pm_wakeup_fw_try)
        ){
            if(pmpriv)
 ...
 */
static cmd_ctrl_node *wlan_get_pending_ioctl_by_id(pmlan_adapter  pmadapter,t_u32 req_id)
{
    cmd_ctrl_node *pcmd_node = MNULL;
 ...
 */
static mlan_status wlan_ret_host_cmd(
    IN  pmlan_private        pmpriv,
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_host_cmd(
    IN pmlan_private        pmpriv,
 ...
 */
static mlan_status wlan_dnld_cmd_to_fw(
    ...
    IN cmd_ctrl_node  *pcmd_node)
{
    ...
    pmadapter->seq_num++;
    pcmd->seq_num = wlan_cpu_to_le16(HostCmd_SET_SEQ_NO_BSS_INFO(pmadapter->seq_num, pcmd_node->priv->bss_num, pcmd_node->priv->bss_type));
    ...
    PRINTM(MCMND, "DNLD_CMD (%lu.%06lu): 0x%x, act 0x%x, len %d, seqno 0x%x\n",
                 sec, usec, cmd_code, wlan_le16_to_cpu(*(t_u16 *)((t_u8 *)pcmd+S_DS_GEN)),
...
static mlan_status wlan_dnld_sleep_confirm_cmd(mlan_adapter *pmadapter)
{
    ...
    static t_u32    i = 0;
    t_u16           cmd_len = 0;
    opt_sleep_confirm_buffer *sleep_cfm_buf = (opt_sleep_confirm_buffer *)(pmadapter->psleep_cfm->pbuf +
    ...
    sleep_cfm_buf->ps_cfm_sleep.seq_num = wlan_cpu_to_le16(
		    HostCmd_SET_SEQ_NO_BSS_INFO(pmadapter->seq_num,
		    (wlan_get_priv(pmadapter,MLAN_BSS_ROLE_ANY))->bss_num,
...
)
{
    ...
    if (pmevent)
        ...
        pcb->moal_recv_event(
            priv->adapter->pmoal_handle, &mevent);
    }
...
mlan_status wlan_alloc_cmd_buffer(IN mlan_adapter *pmadapter)
{
    ...
    for (i = 0; i < MRVDRV_NUM_OF_CMD_BUFFER; i++) {
        if (!(pcmd_array[i].pmbuf = wlan_alloc_mlan_buffer(pmadapter,
                MRVDRV_SIZE_OF_CMD_BUFFER,0,MOAL_MALLOC_BUFFER))) {
...
mlan_status wlan_exec_next_cmd(mlan_adapter *pmadapter)
{
    ...
    pcmd_node = (cmd_ctrl_node *)util_peek_list(
        pmadapter->pmoal_handle, &pmadapter->cmd_pending_q,
        pmadapter->callbacks.moal_spin_lock,
        pmadapter->callbacks.moal_spin_unlock);
...
mlan_status wlan_process_cmdresp(mlan_adapter *pmadapter)
{
    ...
    PRINTM(MCMND, "CMD_RESP (%lu.%06lu): 0x%x, result %d, len %d, seqno 0x%x\n",
            sec, usec, orig_cmdresp_no, cmdresp_result, resp->size, resp->seq_num);
    ...
    if (pmadapter->hw_status == WlanHardwareStatusInitializing) {
        if (ret == MLAN_STATUS_FAILURE) {
            ...
            if (pmadapter->pwarm_reset_ioctl_req) {
                ...
                pcb->moal_ioctl_complete(pmadapter->pmoal_handle,
                    pmadapter->pwarm_reset_ioctl_req, MLAN_STATUS_FAILURE);
    ...
    if ((pmadapter->hw_status == WlanHardwareStatusInitializing) &&
        ...
        while (i < pmadapter->priv_num && !(pmpriv_next = pmadapter->priv[i])) i++;
        ...
        if (!pmpriv_next || i >= pmadapter->priv_num) {
            ...
            if (pmadapter->pwarm_reset_ioctl_req) {
                ...
                pcb->moal_ioctl_complete(pmadapter->pmoal_handle,
                    pmadapter->pwarm_reset_ioctl_req, MLAN_STATUS_SUCCESS);
                pmadapter->pwarm_reset_ioctl_req = MNULL;
                goto done;
            }
#endif
...
t_void wlan_cancel_pending_scan_cmd(pmlan_adapter  pmadapter)
{
    ...
    while ((pcmd_node = wlan_get_pending_ioctl_by_id(pmadapter,MLAN_IOCTL_SCAN)) != MNULL) {
...
t_void wlan_cancel_bss_pending_cmd(pmlan_adapter  pmadapter, t_u32 bss_index)
{
    ...
    wlan_release_cmd_lock(pmadapter);
#ifdef STA_SUPPORT
    if(pmadapter->pext_scan_ioctl_req && (pmadapter->pext_scan_ioctl_req->bss_index == bss_index))
        flash_scan = MTRUE;
    if (flash_scan) {
        /* IOCTL will be completed, avoid calling IOCTL complete again from EVENT */
        pmadapter->pext_scan_ioctl_req = MNULL;
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_ret_rx_mgmt_ind(
    IN  pmlan_private        pmpriv,
    IN  HostCmd_DS_COMMAND   *resp,
    IN  mlan_ioctl_req       *pioctl_buf)
{
 ...
 *  @return             N/A
 */
t_void wlan_host_sleep_activated_event (
    pmlan_private priv,
    t_u8 activated
)
{
 ...
 *  @return             N/A
 */
t_void wlan_host_sleep_wakeup_event (
    pmlan_private priv
)
{
    ENTER();
    ...
    IN mlan_ioctl_req       *pioctl_buf)
{
    ...
    if (wlan_le16_to_cpu(phs_cfg->action) == HS_ACTIVATE) {
       ...
       PRINTM(MCMND, "CMD_RESP: HS_CFG cmd reply result=%#x,"
               ...
               resp->result, phs_cfg->params.hs_config.conditions,
    ...
    IN t_void               *pdata_buf)
{
    ...
    if(cmd_action == DIS_AUTO_PS) {
        ...
        if(ps_bitmap & BITMAP_AUTO_DS) {
            ...
            auto_ps_tlv->header.len = wlan_cpu_to_le16(sizeof(MrvlIEtypes_auto_ds_param_t) - sizeof(MrvlIEtypesHeader_t));
        ...
        if(pdata_buf && (ps_bitmap & (BITMAP_UAP_INACT_PS | BITMAP_UAP_DTIM_PS))) {
            ...
            if(ps_mgmt->flags & PS_FLAG_SLEEP_PARAM) {
                ...
                sleep_tlv->header.len = wlan_cpu_to_le16(sizeof(MrvlIEtypes_sleep_param_t) - sizeof(MrvlIEtypesHeader_t));
                ...
                while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
                    ...
                    switch(tlv_type) {
                        ...
                        case TLV_TYPE_AP_INACT_SLEEP_PARAM:
                            ...
                            pm_cfg->param.ps_mgmt.inact_param.inactivity_to = wlan_le32_to_cpu(inact_tlv->inactivity_to);
    ...
    IN mlan_ioctl_req       *pioctl_buf)
{
    ...
    if (!pmpriv->is_data_rate_auto) {
        pmpriv->data_rate = wlan_index_to_data_rate(pmadapter,
                                pmpriv->tx_rate, pmpriv->tx_htinfo);
    }
        ...
        if(rate->sub_command == MLAN_OID_RATE_CFG) {
            if(rate->param.rate_cfg.rate_type == MLAN_RATE_INDEX) {
                if(pmpriv->tx_htinfo & MBIT(0))
                    ...
                    rate->param.rate_cfg.rate = (pmpriv->tx_rate > MLAN_RATE_INDEX_OFDM0) ?
            ...
            if (pmpriv->tx_htinfo & MBIT(0)) {
                ...
                /* For HostCmd_CMD_802_11_TX_RATE_QUERY, there is a hole in rate table
                   between HR/DSSS and OFDM rates, so minus 1 for OFDM rate index */
                rate->param.data_rate.tx_data_rate = (pmpriv->tx_rate > MLAN_RATE_INDEX_OFDM0) ?
                                                     pmpriv->tx_rate - 1 : pmpriv->tx_rate;
            if (pmpriv->rxpd_htinfo & MBIT(0)) {
                ...
                /* For rate index in RxPD, there is a hole in rate table
                   between HR/DSSS and OFDM rates, so minus 1 for OFDM rate index */
                rate->param.data_rate.rx_data_rate = (pmpriv->rxpd_rate > MLAN_RATE_INDEX_OFDM0) ?
                ...
                IN t_void *pdata_buf)
{
    ...
    cmd->size = wlan_cpu_to_le16(S_DS_GEN + sizeof(HostCmd_DS_TX_RATE_CFG) +
                    sizeof(MrvlRateScope_t) + sizeof(MrvlRateDropPattern_t));
...
mlan_status wlan_adapter_init_cmd (
    ...
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_RECONFIGURE_TX_BUFF,
            HostCmd_ACT_GEN_SET, 0, MNULL, &pmadapter->max_tx_buf_size);
    if (ret) {
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
