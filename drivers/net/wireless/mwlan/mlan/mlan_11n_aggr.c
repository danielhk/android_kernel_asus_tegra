		...
		int pkt_len, int *pad)
{
	...
	*(t_u16 *)(amsdu_buf + amsdu_buf_offset) = mlan_htons(pkt_len +
		LLC_SNAP_LEN - ((2 * MLAN_MAC_ADDR_LENGTH) + sizeof(t_u16)));
...
mlan_status wlan_11n_deaggregate_pkt(mlan_private *priv, pmlan_buffer pmbuf)
{
	...
	while (total_pkt_len >= hdr_len) {
		...
		daggr_mbuf = wlan_alloc_mlan_buffer(pmadapter,pkt_len,0,MOAL_ALLOC_MLAN_BUFFER);
		if (daggr_mbuf == MNULL) {
					...
					int headroom, int ptrindex)
{
	...
	if ((pmbuf_src = (pmlan_buffer) util_peek_list(pmadapter->pmoal_handle, &pra_list->buf_head,
		                                            MNULL, MNULL))) {

		if (!(pmbuf_aggr = wlan_alloc_mlan_buffer(pmadapter,
                        pmadapter->tx_buf_size, 0 ,MOAL_MALLOC_BUFFER))) {
	...
	while (pmbuf_src && ((pkt_size + (pmbuf_src->data_len + LLC_SNAP_LEN)
		...
		pmbuf_src = (pmlan_buffer)
				util_dequeue_list(pmadapter->pmoal_handle, &pra_list->buf_head, MNULL, MNULL);
		...
		pmbuf_src = (pmlan_buffer) util_peek_list(pmadapter->pmoal_handle, &pra_list->buf_head,
		                                            MNULL, MNULL);
	}
