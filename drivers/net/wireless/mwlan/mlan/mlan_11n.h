...
mlan_status wlan_cmd_11n_cfg(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_void *pdata_buf);
 ...
 *  @return	    MTRUE or MFALSE
 */
static INLINE t_u8 is_station_ampdu_allowed(mlan_private *priv,
					raListTbl *ptr, int tid)
{
    sta_node *sta_ptr = MNULL;
 ...
 *  @return	    MTRUE or MFALSE
 */
static INLINE t_u8 wlan_is_cur_bastream_high_prio(mlan_private *priv,
					int tid)
{
	...
	if (!(ptx_tbl = (TxBAStreamTbl *) util_peek_list(priv->adapter->pmoal_handle,
 ...
 *  @return	    MTRUE or MFALSE
 */
static INLINE t_u8 wlan_is_ampdu_allowed(mlan_private *priv,
					raListTbl *ptr, int tid)
{
#ifdef UAP_SUPPORT
    if(GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP)
 ...
 *  @return	    MTRUE or MFALSE
 */
static int INLINE wlan_is_amsdu_in_ampdu_allowed(mlan_private *priv,
					raListTbl *ptr, int tid)
{
	TxBAStreamTbl *ptx_tbl;
	ENTER();
 ...
 *  @return	    MTRUE or MFALSE
 */
static INLINE t_u8 wlan_is_amsdu_allowed(mlan_private *priv,
					raListTbl *ptr, int tid)
{
#ifdef UAP_SUPPORT
    sta_node *sta_ptr = MNULL;
 ...
 *  @return	    MTRUE or MFALSE
 */
static int INLINE wlan_is_bastream_setup(mlan_private *priv,
					raListTbl *ptr, int tid)
{
	TxBAStreamTbl *ptx_tbl;
 ...
 *  @return	    MTRUE or MFALSE
 */
static int INLINE wlan_is_11n_enabled(mlan_private *priv, t_u8 *ra)
{
    int ret = MFALSE;
    ENTER();
#ifdef UAP_SUPPORT
