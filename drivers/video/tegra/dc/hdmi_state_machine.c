...
{
	__cancel_delayed_work(&work_state.dwork);
...
static void handle_recheck_edid_l(struct tegra_dc_hdmi_data *hdmi)
{
	...
	if (hdmi_recheck_edid(hdmi, &match)) {
		 ...
		 */
		if (match) {
			pr_info("No EDID change after HPD bounce,"
				" taking no action.\n");
...
static void hdmi_state_machine_worker(struct work_struct *work)
{
	...
	if (pending_hpd_evt) {
		...
		if (NULL == func)
			pr_warn("NULL state machine handler while in state %d;"
				" how did we end up here?", work_state.state);
		else
			func(work_state.hdmi);
	} else {
		pr_warn("hdmi state machine worker scheduled unexpected state"
			" %d", work_state.state);
	}
}
