...
static struct platform_device molly_nvmap_device __initdata = {
	.name	= "tegra-nvmap",
	.id	= -1,
	.dev	= {
...
int __init molly_panel_init(void)
{
	...
	res = platform_get_resource_byname(&molly_disp2_device,
		IORESOURCE_MEM, "fbmem");
	...
	molly_disp2_device.dev.parent = &phost1x->dev;
	err = platform_device_register(&molly_disp2_device);
	if (err) {
